...
test.describe('Export Excel Flow', () => {
  ...
  test('should reject export for unauthorized role', async ({ page }) => {
    // Requires auth/staff.json — not generated by global-setup (only auth/admin.json = Owner).
    // See Wave 2 task card: multi-role auth generation is a separate task (out of scope).
