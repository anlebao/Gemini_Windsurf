...
public class ShopConfigController : ControllerBase
{
    ...
    private readonly ILogger<ShopConfigController> _logger;

    public ShopConfigController(IShopConfigService shopConfigService, ILogger<ShopConfigController> logger)
    {
    ...
    public ActionResult<ShopConfig> GetShopConfig(Guid shopId)
    {
        try
        {
            var config = _shopConfigService.GetShopConfigAsync(shopId);
            return Ok(config);
        }
        catch (Exception ex)
        {
    ...
    public ActionResult<bool> UpdateShopConfig(Guid shopId, [FromBody] ShopConfig config)
    {
        try
        {
            ...
            var updated = _shopConfigService.UpdateShopConfigAsync(config);
            return Ok(updated);
        }
        catch (Exception ex)
        {
