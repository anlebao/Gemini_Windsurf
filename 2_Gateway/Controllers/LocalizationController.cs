...
public class LocalizationController : ControllerBase
{
    ...
    public async Task<ActionResult<Dictionary<string, string>>> GetStrings()
    {
        try
        {
            var culture = HttpContext.Items["Culture"]?.ToString() ?? _localizationService.GetCulture();
            var strings = await _localizationService.GetAllStringsAsync(culture).ConfigureAwait(false);
    ...
    public async Task<ActionResult<string>> GetString(string key)
    {
        try
        {
            ...
            var culture = HttpContext.Items["Culture"]?.ToString() ?? _localizationService.GetCulture();
            var value = await _localizationService.GetStringAsync(key, culture).ConfigureAwait(false);
    ...
    public async Task<ActionResult<bool>> SetCulture([FromBody] SetCultureRequest request)
    {
        try
        {
            ...
            return Ok(new { Success = true, Culture = request.Culture });
        }
        catch (Exception ex)
        {
