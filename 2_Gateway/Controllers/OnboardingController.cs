    ...
    public class OnboardingController(IOnboardingService onboardingService, ILogger<OnboardingController> logger) : ControllerBase
    {
        ...
        public async Task<ActionResult<bool>> ApplyTemplate(Guid shopId, [FromBody] ApplyTemplateRequest request)
        {
            try
            {
                ...
                OnboardingTemplate? template = await _onboardingService.GetTemplateAsync(templateId);
                OnboardingTemplate result = await _onboardingService.ApplyTemplateAsync(templateId, shopId);

                return result != null
                    ? (ActionResult<bool>)Ok(new { success = true, message = $"Template applied successfully" })
                    : (ActionResult<bool>)BadRequest(new { success = false, message = "Failed to apply template" });
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult<QuickSetupResponse>> QuickSetup(Guid shopId, [FromBody] QuickSetupRequest request)
        {
            try
            {
                ...
                if (applied != null)
                {
                    QuickSetupResponse response = new()
                    {
                        Success = true,
                        ShopId = shopId,
                        ...
                        NextSteps = ["Configure products", "Set up inventory", "Test workflow"]
                    };
