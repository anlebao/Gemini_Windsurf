...
public class WebhookController : ControllerBase
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            // Phase 3.5: Call MarkPaidAsync (not ConfirmPaymentAsync) — Gateway PG only marks status=Paid
            // + enqueues OrderPaymentConfirmed Outbox event. Accounting entries created by ShopERP
            // PaymentConfirmedSubscriber (single source of truth: SQLite).
            await _orderService.MarkPaidAsync(request.OrderId, request.TenantId, request.TransactionId, enqueuePaymentConfirmedEvent: true, cancellationToken);

            return Ok(new { Message = "Payment confirmed. Accounting entries will be generated by ShopERP.", OrderId = request.OrderId });
        }
