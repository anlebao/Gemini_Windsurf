...
public class HKDBookRepository : IHKDBookRepository
{
    ...
    private readonly ILogger<HKDBookRepository> _logger;
    
    public HKDBookRepository(IVanAnDbContext context, ILogger<HKDBookRepository> logger)
    {
        _context = context;
        _logger = logger;
    }
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            var entries = await GetByBookTypeAndPeriodAsync(tenantId, bookType, period, cancellationToken);
            
            var totalDebit = entries.Sum(e => e.Lines.Sum(l => l.DebitAmount));
            var totalCredit = entries.Sum(e => e.Lines.Sum(l => l.CreditAmount));
