    ...
    public class AccountingEntryRepository(IVanAnDbContext context, ILogger<AccountingEntryRepository> logger) : IAccountingEntryRepository
    {
        ...
        CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                IQueryable<CoreAccountingEntry> query = _context.AccountingEntries
                    .Where(e => e.TenantId == tenantId &&
                               e.CreatedAt >= startDate &&
                               e.CreatedAt <= endDate);

                // Execute query with ordering
                List<CoreAccountingEntry> result = await query
                    .OrderByDescending(e => e.CreatedAt)
                    .ToListAsync(cancellationToken);
            ...
            catch (Exception ex) when (ex is not (ArgumentNullException or ArgumentException or OperationCanceledException))
            {
            ...
            CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                DateTime startDate = DateTime.SpecifyKind(period.StartDate, DateTimeKind.Utc);
                DateTime endDate = DateTime.SpecifyKind(period.EndDate, DateTimeKind.Utc);
        ...
        public async Task AddAsync(CoreAccountingEntry entry, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                CoreAccountingEntry? existingEntry = await _context.AccountingEntries
                    .AsNoTracking()
            ...
            catch (Exception ex) when (ex is not (ArgumentNullException or ArgumentException or InvalidOperationException or OperationCanceledException))
            {
        ...
        public async Task AddRangeAsync(IEnumerable<CoreAccountingEntry> entries, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                List<CoreAccountingEntry> entriesList = entries.ToList();
                ...
                TenantId firstTenantId = entriesList.First().TenantId ?? throw new InvalidOperationException("First entry has null TenantId");
                List<CoreAccountingEntry> mixedTenantEntries = entriesList.Where(e => e.TenantId != firstTenantId).ToList();
                if (mixedTenantEntries.Count != 0)
                {
                    IEnumerable<string> mixedTenantIds = mixedTenantEntries.Select(e => e.TenantId.Value.ToString()).Distinct();
                    throw new InvalidOperationException(
                ...
                List<Guid> duplicateIds = entriesList
                    .GroupBy(e => e.Id)
                    .Where(g => g.Count() > 1)
                    .Select(g => g.Key)
                    .ToList();
            ...
            catch (Exception ex) when (ex is not (ArgumentNullException or InvalidOperationException or OperationCanceledException))
            {
            ...
            CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                DateTime startDate = DateTime.SpecifyKind(new DateTime(period.Year, period.Month, 1), DateTimeKind.Utc);
                DateTime endDate = DateTime.SpecifyKind(startDate.AddMonths(1).AddDays(-1), DateTimeKind.Utc);
            ...
            catch (Exception ex) when (ex is not (ArgumentNullException or OperationCanceledException))
            {
