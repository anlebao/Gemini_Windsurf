...

{
    ...
    /// <inheritdoc />
    public async Task<PagedResult<EligibleCustomerDto>> GetActiveCollaboratorsForTenantAsync(Guid tenantId, int page, int pageSize)
    {
        if (page < 1) page = 1;
        if (pageSize < 1) pageSize = 20;
        if (pageSize > 100) pageSize = 100;
        ...
        // #185-6: active roles of this tenant — scalar projection (no PII decrypt).
        var roleRows = await _dbContext.CommunityRoles
            .IgnoreQueryFilters()
            .AsNoTracking()
            .Where(r => r.TenantId == tenantIdObj && r.IsActive)
            .Select(r => new { r.CustomerId, RoleType = r.RoleType.ToString(), r.ActivatedAt })
            .ToListAsync();

        var grouped = roleRows
            .GroupBy(r => r.CustomerId)
            .Select(g => new
            {
                CustomerId = g.Key,
                Roles = g.Select(x => x.RoleType).Distinct().ToList(),
                FirstActivatedAt = g.Min(x => x.ActivatedAt)
            })
            .OrderBy(g => g.FirstActivatedAt)
            .ToList();

        int total = grouped.Count;
        var pageItems = grouped.Skip((page - 1) * pageSize).Take(pageSize).ToList();
        var customerIds = pageItems.Select(g => g.CustomerId).ToList();

        // Scalar projection — PhoneNumber/Email NOT selected (encrypted converters would
        // materialize and can throw CryptographicException on legacy rows).
        var customers = await _dbContext.Customers
            .IgnoreQueryFilters()
            .AsNoTracking()
            .Where(c => customerIds.Contains(c.Id))
            .Select(c => new { c.Id, c.FullName, c.LoyaltyPoints, IdentityLevel = c.IdentityLevel.ToString() })
            .ToListAsync();

        var items = pageItems
            .Join(customers, g => g.CustomerId, c => c.Id, (g, c) => new EligibleCustomerDto
            {
                ...
                LoyaltyPoints = c.LoyaltyPoints,
                IdentityLevel = c.IdentityLevel,
                ExistingRoles = g.Roles,
                FirstActivatedAt = g.FirstActivatedAt
            })
            .ToList();
