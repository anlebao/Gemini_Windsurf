...
public static class AccountCodeValidator
{
    ...
    public static bool IsValidVNAccountCode(string accountCode)
    {
        ...
        if (!int.TryParse(accountCode, out var code)) return false;
        if (accountCode.Length != 3) return false;
    ...
    {
        if (string.IsNullOrWhiteSpace(accountCode) || !int.TryParse(accountCode, out var code))
