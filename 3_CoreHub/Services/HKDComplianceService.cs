...
/// </summary>
public class HKDComplianceService : IHKDComplianceService
{
    private readonly ILogger<HKDComplianceService> _logger;
    private readonly IHKDTaxClassificationService _taxClassificationService;
    
    public HKDComplianceService(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var rules = await GetComplianceRulesAsync(bookType, cancellationToken);
        var violations = new List<ComplianceViolation>();
        var warnings = new List<ComplianceWarning>();
        
        foreach (var rule in rules)
        {
            var result = await ValidateRuleAsync(tenantId, bookType, rule, period, cancellationToken);
            violations.AddRange(result.Violations);
            warnings.AddRange(result.Warnings);
        }
        
        var isCompliant = violations.Count == 0;
        var complianceScore = CalculateComplianceScore(rules.Count, violations.Count, warnings.Count);
        var status = DetermineComplianceStatus(isCompliant, violations, warnings);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        rules.AddRange(GetBaseTT152Rules());
        
        // Book type specific rules
        rules.AddRange(GetBookTypeSpecificRules(bookType));
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var complianceResult = await ValidateBookComplianceAsync(tenantId, bookType, period, cancellationToken);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var isDataIsolated = violations.Count == 0;
        var isolationScore = isDataIsolated ? 100 : 50;
    ...
    private async Task<(List<ComplianceViolation> Violations, List<ComplianceWarning> Warnings)> ValidateRuleAsync(
        TenantId tenantId,
        AccountingBookType bookType,
        ComplianceRule rule,
        DateTime period,
        CancellationToken cancellationToken)
    {
        ...
        if (rule.RuleId.StartsWith("S1a"))
        {
            ...
            if (rule.RuleId == "S1a-001")
            {
                ...
                var isVATRegistered = false; // Simulated
                if (isVATRegistered)
                {
    ...
    private async Task ValidateTransactionRulesAsync(
        TenantId tenantId,
        AccountingBookType bookType,
        CoreAccountingEntry entry,
    ...
    private decimal CalculateComplianceScore(int totalRules, int violations, int warnings)
    {
        if (totalRules == 0) return 100;
        
        var passedRules = totalRules - violations;
        var baseScore = (decimal)passedRules / totalRules * 100;
        var warningDeduction = warnings * 2; // 2 points per warning
        
        return Math.Max(0, baseScore - warningDeduction);
    }
    
    private ComplianceStatus DetermineComplianceStatus(bool isCompliant, List<ComplianceViolation> violations, List<ComplianceWarning> warnings)
    {
        if (isCompliant) return ComplianceStatus.Compliant;
        if (violations.Any(v => v.Severity == SeverityLevel.High)) return ComplianceStatus.NonCompliant;
        if (violations.Any(v => v.Severity == SeverityLevel.Medium)) return ComplianceStatus.PartiallyCompliant;
        return ComplianceStatus.RequiresReview;
    }
    
    private List<Recommendation> GenerateRecommendations(HKDComplianceResult complianceResult)
    {
        var recommendations = new List<Recommendation>();
        
        foreach (var violation in complianceResult.Violations)
        {
