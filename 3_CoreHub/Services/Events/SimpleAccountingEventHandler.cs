...
public class SimpleAccountingEventHandler : BackgroundService
{
    ...
    private readonly ILogger<SimpleAccountingEventHandler> _logger;

    public SimpleAccountingEventHandler(
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        try
        {
            ...
            var subscription = connection.SubscribeAsync("vanan.events.ordercompleted", async (sender, args) =>
            {
    ...
    private async Task HandleOrderCompletedEventAsync(Msg message, CancellationToken cancellationToken)
    {
        try
        {
            var eventData = Encoding.UTF8.GetString(message.Data);
            var orderEvent = JsonSerializer.Deserialize<OrderCompletedEvent>(eventData, new JsonSerializerOptions
            {
            ...
            var accountingEntry = await accountingService.CreateEntryAsync(new AccountingEntryDto
            {
            ...
            var hkdService = scope.ServiceProvider.GetRequiredService<IHKDBookService>();
            var coreEntry = ConvertToCoreAccountingEntry(accountingEntry);
    ...
    /// </summary>
    private AccountingEntry ConvertToCoreAccountingEntry(AccountingEntryDto dto)
    {
    ...
    {
        var natsUrl = "nats://localhost:4222"; // TODO: Make configurable
        
        try
        {
            var connection = new ConnectionFactory().CreateConnection(natsUrl);
