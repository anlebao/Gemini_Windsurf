...
### **Git Push Procedure (Windows PowerShell)**

**FAST PUSH (preferred — when CI already validated in session):**
- Use `git push --no-verify origin main` to skip the pre-push hook (~17 min CI rerun)
- **Condition:** MUST have already run `guard-check.ps1` + `dotnet build VanAn.sln` + `dotnet test` (Core.Tests) in the current session, ALL PASS
- **Verify:** `gh api repos/anlebao/Gemini_Windsurf/branches/main --jq '.commit.sha'` vs `git rev-parse HEAD`

**STANDARD PUSH (full pre-push CI):**
- **Step 1:** `$env:GH_TOKEN="<token>"` — set token in session FIRST
- **Step 2:** `gh auth setup-git` — configure credential helper
- **Step 3:** `git push origin main` — push with non-interactive credential helper
- Pre-push hook runs `scripts/ci-full.ps1 -SkipE2E` (~12 min — E2E temporarily skipped)

**CREDENTIAL HELPER FIX (if push hangs with no output):**
- Root cause: `credential.helper=store` in `~/.gitconfig` + `GH_TOKEN` not visible to git subprocess
- Fix: `git -c credential.helper=manager git push origin main` (use manager helper explicitly)
- Or: `git config --global credential.helper manager` (one-time, persists)
- **NEVER** embed token in `remote.origin.url` — causes credential helper conflicts + hangs
- **NEVER** use `gh run watch` to wait for CI — it hangs for the same credential reason. Use `gh run list --json status,conclusion` polling instead
- **If push hangs:** restore clean URL `git remote set-url origin "https://github.com/<owner>/<repo>.git"`, ensure `GH_TOKEN` is set, retry
- **Verify push success:** `gh api repos/<owner>/<repo>/branches/main --jq '.commit.sha'` vs `git rev-parse HEAD`
- **Detailed pattern:** See Known Error Pattern #11 in this file
...
| 10 | `The format of value 'application/json; charset=utf-8' is invalid.` (FormatException) when forwarding request body in Gateway controllers | `StringContent(body, Encoding.UTF-, mediaType)` and `new MediaTypeHeaderValue(mediaType)` BOTH reject the `; charset=utf-8` suffix — the mediaType parameter accepts ONLY the media type; charset must be set via the `Encoding` parameter (StringContent) or the `MediaTypeHeaderValue.CharSet` property. Browsers/fetch send `Content-Type: application/json; charset=utf-8` by default, so any Gateway forward controller passing `Request.ContentType` straight through throws at runtime on EVERY POST. | Strip charset before passing: `var mediaType = (Request.ContentType ?? "application/json").Split(';', StringSplitOptions.TrimEntries)[0];` then use `mediaType` in `StringContent(body, Encoding.UTF8, mediaType)` OR `new MediaTypeHeaderValue(mediaType)`. NEVER pass `Request.ContentType` raw to either constructor. **Audit all Gateway forward controllers** — both `StringContent` and `StreamContent + MediaTypeHeaderValue` variants are affected. **Reference implementation:** `RedemptionController`, `LoyaltyController`, `CustomerIdentityController`, `CustomerProfileController`, `MissionsController` (fix #106, 2026-08-06). | Gateway Fix #106 |
| 11 | `git push origin main` hangs indefinitely (no output, no error, never completes) on Windows PowerShell | Four root causes: (a) **Credential helper conflict** — `~/.gitconfig` has `credential.https://github.com.helper=!'C:\Program Files\GitHub CLI\gh.exe' auth git-credential` which spawns `gh.exe` as a subprocess. If `gh.exe` is not in PATH or `GH_TOKEN` env var is not set in the shell session, the subprocess hangs waiting for interactive input that never comes (PowerShell pipes don't provide a TTY). (b) **`credential.helper=store` conflict** — `~/.gitconfig` has `credential.helper=store` which reads `~/.git-credentials` file. If the stored credential is stale or for a different account, git hangs waiting for a fresh credential that never arrives in non-interactive mode. (c) **Remote URL token embedding** — manually setting `git remote set-url origin "https://x-access-token:<token>@github.com/..."` conflicts with credential helpers that try to negotiate credentials anyway, causing double-auth hangs. (d) **Pre-push hook running full CI** — `scripts/ci-full.ps1` takes ~12-17 min; push appears to "hang" but is actually running CI. **NOT a credential issue.** | **Correct push procedure:** (1) Set `$env:GH_TOKEN="<token>"` in the PowerShell session FIRST. (2) Run `gh auth setup-git` to configure credential helper with the token. (3) Run `git push origin main` — credential helper will use the token non-interactively. (4) **NEVER** modify `remote.origin.url` to embed tokens — this causes credential helper conflicts. (5) If push still hangs, verify with `git remote set-url origin "https://github.com/<owner>/<repo>.git"` (clean URL, no token) then retry. (6) **Credential helper fix:** `git -c credential.helper=manager git push origin main` (use manager helper explicitly, bypasses `store` stale cred). Or one-time: `git config --global credential.helper manager`. (7) **Fast push (skip pre-push CI):** If CI already validated in session (guard-check + build + tests PASS), use `git push --no-verify origin main` — skips the ~12 min pre-push hook. (8) To verify push succeeded without waiting on hanging `gh run watch`: use `gh api repos/<owner>/<repo>/branches/main --jq '.commit.sha'` and compare with `git rev-parse HEAD`. **Reference:** Charity checkout session 2026-09-14 — push appeared to hang but was actually running pre-push CI (1154s); credential.helper=store caused earlier attempts to fail with "invalid credentials". | Git Push 2026-09-12 + 2026-09-14 |

| 12 | `error CS0854: An expression tree may not contain a call or invocation that uses optional arguments` (Moq Setup/Verify) after adding an OPTIONAL parameter to an interface method | Moq `Setup`/`Verify` lambdas ARE expression trees; a call site that omits trailing optional args is legal in normal code but ILLEGAL in an expression tree. ANY existing `Setup(w => w.Method(...))`/`Verify(...)` that lists fewer args than the new full signature fails to compile — even when the new param has a default. | After changing an interface signature: `grep -rn "Setup(w => w.<Method>(\|Verify(w => w.<Method>("` across ALL test files and append the new argument (usually `It.IsAny<Guid>()`) to every Setup/Verify expression. Direct invocations (non-expression) are fine with omitted optional args. Batch 5 BUG-1 hit this in 8 files (`OrderWorkflowAllianceTests`, `RedemptionAllianceTests`, `RedemptionCancelAllianceTests`, `LoyaltyRewardsActivateAllianceTests`, `MissionServiceAllianceTests`, `LoyaltyPointLedgerServiceTests`, …). | Loyalty BUG-1 |
| 13 | `grep` markers on deployed DLLs return 0 (or false positives) for STRING LITERALS — L2 "code deployed" check fails/ambiguous | .NET metadata has TWO string heaps: **#Strings (UTF-8)** holds type/member names — grep-able; **#US (UTF-16)** holds string LITERALS (route templates, datetime formats, json fields) — NOT grep-able as contiguous ASCII (interleaved `\0`). Route prefixes like `api/platform/loyalty` may match only because the same text also appears in UTF-8 metadata. Literals like `yyyy-MM-ddTHH:mm:ss.fffffffZ` NEVER match. | For L2 deploy verification prefer: (a) **type/method NAME markers** (UTF-8 metadata — reliable, e.g. `GetTenantBalancesAsync`, `ComputeNetEarnByTenantAsync`); (b) **BEHAVIORAL verification** (e.g. check the actual runtime output format — history timestamps now contain fractional seconds → publisher fix deployed); (c) route strings ONLY when confirmed by a known-good prior grep. NEVER trust grep on datetime format literals / pure string constants. | Loyalty Ordering Guard RV |
| 14 | `gcloud compute ssh <vm> --zone … --command 'bash -s' < script.sh` on Windows mangles stdin (`bash: line 1: y: command not found`, garbage bytes) | gcloud uses **plink.exe** (PuTTY) on Windows; `< file` stdin piping into the remote `bash -s` is unreliable — plink consumes stdin as terminal input, not the script. | Use the **scp + run** pattern instead: `gcloud compute scp script.sh <vm>:/tmp/x.sh --zone … --project …` then `gcloud compute ssh <vm> --zone … --project … --command "bash /tmp/x.sh"`. For DBs inside docker volumes: find the HOST path with `docker inspect <container> --format '{{range .Mounts}}{{.Source}} -> {{.Destination}}{{println}}{{end}}'` (e.g. ShopERP DB host path = `/var/lib/docker/volumes/vanan_shoperp_data/_data/vanan_shoperp.db`, NOT `/app/keys` which only exists inside the container); access with `sudo python3 -c "import sqlite3; …"` (host python3 + sqlite3 works; root needed for write). | Loyalty RV |
| 15 | Consumer sees events OUT OF ORDER despite publishing on ONE NATS subject | Outbox-backed events are published TWICE by design (direct NATS + NatsSyncWorker from Outbox — two independent channels). The two channels interleave → a consumer CAN process a REDEEM before the earlier EARNs even with per-subject FIFO on each channel. Any consumer that applies CUMULATIVE state (balance overwrite, "last wins") from such events needs an explicit ordering guard. | (a) Publisher: emit a full-precision timestamp (`yyyy-MM-ddTHH:mm:ss.fffffffZ`) — second-precision makes same-second events indistinguishable. (b) Consumer: only apply state when the event is NOT older than the newest entry already applied (compare vs max timestamp of the audit history; stale events still append their audit entry but cannot clobber). NEVER rely on delivery order. | Loyalty Ordering Guard |
| 16 | `Assert.Equal(0, actual, "message")` → `error CS1503: cannot convert from 'string' to 'System.Func<int,int,bool>'` (xUnit) | xUnit's 3-arg `Assert.Equal` is `(T expected, T actual, IEqualityComparer<T> comparer)` — there is NO string-message overload. Same trap for `Assert.Single(…, "msg")`. | Use `Assert.True(condition, "message")` (or FluentAssertions `.Should().Be(x, "because")`). Applies to ALL xUnit equality assertions. | Loyalty tests |
| 17 | "Protective" merge semantics (e.g. MAX-merge "never decrease") become correctness bugs after the WRITE path moves to a new authority | Batch 1 used MAX-merge on the SQLite mirror to protect POS points pre-cutover (PG not yet authority). After Batch 2 made PG the single source of truth (all writes route through the ledger + backfill ran), MAX-merge froze the mirror at the last high-water mark forever — SPEND events could never decrease it (upward drift: mirror 150 vs PG 70). The plan (T2.2) said "mirror overwrite là đúng" but the implementation kept the old guard. | Whenever the write path is migrated to a new authority (cutover), REVISIT every "protective" read-path merge/sync semantic AND its tests (tests codify the old contract — they must be rewritten, e.g. LPI-B1-7). The mirror must OVERWRITE with the authoritative value; "never decrease" guards only make sense when the mirror can hold values the authority doesn't know yet. | Loyalty BUG-1b |
