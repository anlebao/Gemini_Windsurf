...
curl -sk -D /tmp/headers_otp2 -X POST -H 'Content-Type: application/json' \
  -d "{\"phoneNumber\":\"$PHONE\"}" \
  https://api.khachvip.online/api/customers/otp/send
echo
OTP=$(grep -i 'X-Dev-OTP' /tmp/headers_otp2 | tr -d '\r' | awk '{print $2}')
echo "OTP: $OTP"
...
VERIFY_RESP=$(curl -sk -X POST -H 'Content-Type: application/json' \
  -d "{\"phoneNumber\":\"$PHONE\",\"otp\":\"$OTP\",\"DisplayName\":\"Fresh Mission Test\"}" \
