...
namespace VanAn.Core.Tests.TestInfrastructure
{
    ...
    public static class TestEntityBuilder
    {
        ...
        // Tests now use TenantId directly without needing a Shop entity.
        ...
        /// </summary>
        public static TenantId CreateTenantId(Guid? id = null)
        {
            return new TenantId(id ?? Guid.Parse("12345678-1234-1234-1234-123456789abc"));
        }

        /// <summary>
        /// Creates a test CustomerId for testing
        /// </summary>
        public static CustomerId CreateCustomerId(Guid? id = null)
        {
            return new CustomerId(id ?? Guid.NewGuid());
        }

        /// <summary>
        /// Creates a test ProductId for testing
        /// </summary>
        public static ProductId CreateProductId(Guid? id = null)
        {
            return new ProductId(id ?? Guid.NewGuid());
        }
        ...
        public static AccountingEntry CreateAccountingEntry(TenantId tenantId, AccountingEntryType type, Money amount, AccountingPeriod? period = null)
        {
            ...
            string description = type == AccountingEntryType.Revenue ? "Test Revenue Entry" : "Test Expense Entry";

            return type == AccountingEntryType.Revenue
                ? AccountingEntry.CreateRevenue(tenantId, period ?? AccountingPeriod.FromDateTime(DateTime.UtcNow), amount, description)
                : AccountingEntry.CreateExpense(tenantId, period ?? AccountingPeriod.FromDateTime(DateTime.UtcNow), amount, description);
        }
        ...
        {
            AccountingEntry originalEntry = CreateAccountingEntry(tenantId, AccountingEntryType.Revenue, amount);
            AccountingEntry reversalEntry = CreateAccountingEntry(tenantId, AccountingEntryType.Expense, new Money(-amount.ToDecimal()), originalEntry.Period);
