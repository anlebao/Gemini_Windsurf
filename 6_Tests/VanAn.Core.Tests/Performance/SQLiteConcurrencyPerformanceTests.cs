...
namespace VanAn.Core.Tests.Performance
{
    ...
    public class SQLiteConcurrencyPerformanceTests : IDisposable
    {
        ...
        public async Task ThroughputTest_ShouldProcess100Orders_WithinTimeLimit()
        {
            ...
            int orderCount = 100;
            TimeSpan timeLimit = TimeSpan.FromSeconds(10); // Actual processing time after removing fixed delay
            ...
            int processedOrders = metrics.ProcessedBatches * 8; // 8 orders per batch
        ...
        public async Task LatencyTest_ShouldProcessOrders_WithAcceptableLatency()
        {
            ...
            TenantId tenantId = new(Guid.NewGuid());
            int orderCount = 50;
            TimeSpan maxLatency = TimeSpan.FromSeconds(5);
            ...
            for (int i = 0; i < orderCount; i++)
            {
                ...
                Order order = CreateTestOrder(tenantId, $"Latency Order {i}");
                order.UpdateOrderStatus(new OrderStatusId("completed"));
            ...
            // Assert
            TimeSpan avgLatency = TimeSpan.FromTicks((long)latencies.Average(l => l.Ticks));
            TimeSpan maxObservedLatency = latencies.Max();
        ...
        public async Task ConcurrencyTest_ShouldHandleMultipleConcurrentEnqueues()
        {
            ...
            int totalOrders = concurrentThreads * ordersPerThread;
            ...
            for (int thread = 0; thread < concurrentThreads; thread++)
            {
                ...
                tasks.Add(Task.Run(async () =>
                {
                    ...
                    {
                        Order order = CreateTestOrder(tenantId, $"Concurrent Order {threadId}-{i}");
                        order.UpdateOrderStatus(new OrderStatusId("completed"));
                        await _queueService.EnqueueOrderAsync(order);
                    }
                }));
            ...
            // Assert
            QueueMetrics metrics = await _queueService.GetQueueMetricsAsync();
            int processedOrders = metrics.ProcessedBatches * 8;
        ...
        public async Task MemoryUsageTest_ShouldNotExceedMemoryLimits()
        {
            ...
            TenantId tenantId = new(Guid.NewGuid());
            int orderCount = 200;
            int maxMemoryMB = 100; // 100MB limit
            ...
            for (int i = 0; i < orderCount; i++)
            {
                Order order = CreateTestOrder(tenantId, $"Memory Order {i}");
                order.UpdateOrderStatus(new OrderStatusId("completed"));
                await _queueService.EnqueueOrderAsync(order);
            }
        ...
        public async Task LongRunningTest_ShouldMaintainPerformance_OverTime()
        {
            ...
            while (stopwatch.Elapsed < testDuration)
            {
                try
                {
                    Order order = CreateTestOrder(tenantId, $"LongRun Order {ordersProcessed}");
                    order.UpdateOrderStatus(new OrderStatusId("completed"));
                    await _queueService.EnqueueOrderAsync(order);
                    ordersProcessed++;
            ...
            // Assert
            QueueMetrics metrics = await _queueService.GetQueueMetricsAsync();
            int actualProcessed = metrics.ProcessedBatches * 8;
        ...
        public async Task BatchEfficiencyTest_ShouldOptimizeBatchProcessing()
        {
            // Arrange
            TenantId tenantId = new(Guid.NewGuid());
            int[] batchSizes = [4, 8, 16, 32];
            List<(int batchSize, TimeSpan processingTime)> results = [];
            ...
            foreach (int batchSize in batchSizes)
            {
                ...
                IEnumerable<Task> enqueueTasks = orders.Select(async order => await _queueService.EnqueueOrderAsync(order));
                await Task.WhenAll(enqueueTasks);
            ...
            foreach ((int batchSize, TimeSpan processingTime) in results)
            {
                double throughput = batchSize * 2 / processingTime.TotalSeconds;
            ...
            (int batchSize, TimeSpan processingTime) optimalBatch = results.OrderBy(r => r.processingTime).First();
            Assert.True(optimalBatch.batchSize is >= 4 and <= 32,
