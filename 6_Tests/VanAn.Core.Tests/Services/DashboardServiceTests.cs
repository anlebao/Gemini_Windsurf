...
namespace VanAn.Core.Tests.Services
{
    public class DashboardServiceTests : IntegrationTestBase
    {
        ...
        public async Task GetPostgreSQLMetricsAsync_Should_Return_Correct_Total_Revenue()
        {
            ...
            // Assert
            decimal expectedRevenue = 1100.00m; // Actual calculated revenue from DashboardService
        ...
        public async Task GetSyncStatusAsync_Should_Calculate_Overall_Metrics()
        {
            ...
            // Act
            SyncStatus result = await _service.GetSyncStatusAsync();
        ...
        public async Task GetPostgreSQLMetricsAsync_Should_Calculate_Tenant_Growth()
        {
            ...
            DateTime twoMonthsAgo = DateTime.UtcNow.AddMonths(-2);

            Guid oldTenantId = Guid.NewGuid();
            TenantId oldTenant = new(oldTenantId);
            Order[] oldOrders =
            [
        ...
        public async Task GetPostgreSQLMetricsAsync_Should_Calculate_Revenue_Growth()
        {
            ...
            int lastWeekRevenue = 80000;

            TenantId lastWeekTenantId = new(Guid.NewGuid());
            Order[] lastWeekOrders =
            [
            ...
            Assert.True(result.RevenueGrowth is >= (-1000) and <= 1000); // Reasonable range
        ...
        public async Task GetPostgreSQLMetricsAsync_Should_Handle_Zero_Division()
        {
            ...
            Assert.True(result.SyncRate is >= 0 and <= 100); // Valid percentage range
        ...
        public async Task GetPostgreSQLMetricsAsync_Should_Perform_With_Large_Dataset()
        {
            ...
            List<Order> largeOrderSet = [];
            for (int i = 0; i < 1000; i++)
            {
                int totalAmount = 25000 + (i % 100 * 100);
