...
> **Má»¥c Ä‘Ã­ch:** LÆ°u trá»¯ cÃ¡c wave Ä‘Ã£ hoÃ n thÃ nh Ä‘á»ƒ giáº£m file size cá»§a project_state.md
...
**QuickSetup + Product Management â€” Phases 4â€“6**

**Status:** COMPLETED (2026-07-17) â€” merged to `main`
...
**PREVIOUS OBJECTIVE (archived)**
**Single-Identity Refactor (HÆ°á»›ng A) â€” all affected entities + VPS crash fix**

**Status:** COMPLETED + VPS VERIFIED (2026-07-17) â€” merged to `main`
...
**PREVIOUS OBJECTIVE (archived)**
**Wave 8â€“16 Production Hygiene + Wave 16 Production Hardening + Pre-Wave 17 Fixes**

**Status:** âœ… COMPLETED (2026-06-24 â†’ 2026-06-28) â€” All merged to `main`

**Completed Actions:**
1. âœ… Wave 8: Upgrade Dashboard to Sitemap with Authentication (commit `d088739`)
2. âœ… Wave 9: Cleanup Orphan Controller â€” deleted `ShopERP/Controllers/CustomersController.cs`
3. âœ… Wave 10: Cleanup Duplicate Interfaces â€” deleted `ISocialCampaignService`/`ILoyaltyRewardsService` duplicates in ShopERP
4. âœ… Wave 11: Cleanup Invalid Framework Files â€” deleted `SocialCampaignManager.cshtml`, `KhachLink/wwwroot/index.html`
5. âœ… Wave 12: Fix API Authorization â€” `[Authorize(Policy="RequireTenantAccess")]` on Gateway + ShopERP endpoints
6. âœ… Wave 13: Replace Hardcoded Data â€” public `GET /api/products?shopId=` + `ProductHttpService` via Gateway
7. âœ… Wave 14: HMAC Request Signing â€” `HmacSigningMiddleware` + `ApiKey` entity + `IApiKeyManagementService`
8. âœ… Wave 15: KhachLink Page Cleanup + Blazor Web App routing (commit `26abd83`)
9. âœ… Wave 16: Production flow hardening â€” Campaign, Dashboard TenantId, VoiceCommand
10. âœ… Production fixes: resolved 502 errors (ShopERP stale volume + KhachLink 502)
11. âœ… Customer API Integration Tests Fix â€” 100% success rate (2026-06-24)
12. âœ… All integration tests: 144/144 PASS (2026-06-28)
...
**PREVIOUS OBJECTIVE (archived)**
**Production Hygiene â€” Wave 7: Production Hardening**

**Status:** âœ… COMPLETED â€” Branch `feature/wave7-prod-hardening`, merged to base for Wave 8

**Completed Actions:**
1. âœ… W7-T1 through W7-T5: Production hardening tasks (see PRODUCTION_HYGIENE_master_plan.md for details)
...
**PREVIOUS OBJECTIVE (archived)**
**Security Compliance â€” Wave 6: User Aggregate + RBAC Management**

**Status:** âœ… COMPLETED â€” Branch `feature/wave6-user-rbac-mgmt`, merged to `main` (commit `2599c1b`)

**Completed Actions:**
1. âœ… W6-T1: `1_Shared/Domain/Aggregates/UserAggregate/DemoUser.cs` (AggregateRoot lifecycle: Create, Deactivate, Reactivate, ChangePassword, AssignRole, UpdateProfile)
2. âœ… W6-T2: `UserRole.cs`, `UserTenant.cs`, `PermissionGroup.cs`, `UserPermissionGroup.cs`, `UserEvents.cs`
3. âœ… W6-T3: Legacy `DemoUser`, `UserTenant`, `UserRole` in `Domain.cs` marked `[Obsolete]`
4. âœ… W6-T4: `IUserManagementService` + `UserManagementService` (Create/List/Get/Update/Deactivate/Reactivate/ChangePassword)
5. âœ… W6-T5: `IRoleAssignmentService` + `RoleAssignmentService` (assign/revoke roles, group membership, effective roles)
6. âœ… W6-T6: `IPermissionGroupService` + `PermissionGroupService` (create/update/list groups, add/remove roles)
7. âœ… W6-T7: `UserController` in `ShopERP/Controllers/` â€” tenant-scoped CRUD endpoints
8. âœ… W6-T8: `PermissionGroupController` in `ShopERP/Controllers/` â€” group CRUD endpoints
9. âœ… W6-T9: `UserCreatedEvent` handler dispatches welcome email via `INotificationService`
10. âœ… W6-T10: `UserManagement.razor` at `/admin/users`
11. âœ… W6-T11: `PermissionGroupManagement.razor` at `/admin/permission-groups` + NavMenu entries
12. âœ… W6-T12: `UserDomainTests` (7 cases) + `UserManagementServiceTests` (9) + `RoleAssignmentServiceTests` (6) + `PermissionGroupServiceTests` (7) = 29/29 PASS
...
**PREVIOUS OBJECTIVE (archived)**
**Security Compliance â€” Wave 5: Domain Refactor (God File Split) + Tenant Rich Domain Model + Tenant CRUD**

**Status:** âœ… COMPLETED â€” Branch `feature/wave5-tenant-mgmt`, merged into `feature/wave6-user-rbac-mgmt` base

**Completed Actions:**
1. âœ… W5-T1: `AggregateRoot` base + `IDomainEvent` interface added to `Common.cs`
2. âœ… W5-T2: `1_Shared/Domain/Aggregates/TenantAggregate/Tenant.cs` (Rich Domain) + `TenantStatus.cs` + `TenantSettings.cs`
3. âœ… W5-T3: `TenantAggregate/TenantEvents.cs` â€” `TenantCreatedEvent`, `TenantSuspendedEvent`, `TenantDeactivatedEvent`
4. âœ… W5-T4: `record Tenant` in `Domain.cs` marked `[Obsolete]`; `TenantConfiguration.cs` updated; `IVanAnDbContext` + `VanAnDbContext.Tenants` now typed to new aggregate; integration tests migrated
5. âœ… W5-T5: `ITenantManagementService` + `TenantManagementService` (Create/List/Get/Update/Suspend/Reactivate/Deactivate)
6. âœ… W5-T6: `TenantController` in `ShopERP/Controllers/` â€” 7 endpoints; `SystemAdmin` policy added to Gateway + ShopERP
7. âœ… W5-T7: `TenantCreatedEvent` handler dispatches welcome email via `INotificationService`
8. âœ… W5-T8: `3_CoreHub/EmailTemplates/TenantWelcomeEmail.html` (Vietnamese template)
9. âœ… W5-T9: `TenantManagement.razor` at `/admin/tenants` â€” list/create/suspend/reactivate/deactivate; NavMenu entry for `SystemAdmin`
10. âœ… W5-T10: `TenantDomainTests` (13 cases) + `TenantManagementServiceTests` (10 cases)
...
**PREVIOUS OBJECTIVE (archived)**
**Security Compliance â€” Wave 4: RBAC Enforcement at Blazor UI Layer**

**Status:** âœ… COMPLETED â€” Branch `feature/wave4-rbac-ui`, merged to `main` (commit `5a6b441`)

**Completed Actions:**
1. âœ… W4-T1 through W4-T6: AuthorizeRouteView, policy-gated pages, NavMenu role gates, AccessDenied.razor, role-based login redirect, E2E tests
...
**PREVIOUS OBJECTIVE (archived)**
**Security Compliance â€” Wave 3: Report Export (Excel with EPPlus)**

**Status:** âœ… COMPLETED â€” Branch `feature/wave3-report-export`, merged to `main` (PR #42 merged wave 3)

**Completed Actions:**
1. âœ… W3-T1 through W3-T8: EPPlus export, ReportController, E2E tests, unit tests
...
**PREVIOUS OBJECTIVE (archived)**
**Security Compliance â€” Wave 2: Data Protection (Field-level Encryption)**

**Status:** âœ… COMPLETED â€” Branch `feature/wave2-data-protection`, merged to `main`

**Completed Actions:**
1. âœ… W2-T1: `AddDataProtection()` registered in `3_CoreHub/Program.cs` + `5_WebApps/ShopERP/Program.cs`, keys persisted to `./keys/`
2. âœ… W2-T2: `EncryptedStringConverter` â€” EF Core ValueConverter using `IDataProtector`
3. âœ… W2-T3: `EncryptedStringConverter` applied to `CustomerConfiguration.cs` â€” PhoneNumber, Email
4. âœ… W2-T4: `EncryptedStringConverter` applied to `LeadConfiguration.cs` + `FacebookLeadConfiguration.cs` â€” PhoneNumber, Email
5. âœ… W2-T5: EF Core Migration created â€” columns resized to `HasMaxLength(500)` for encrypted values
6. âœ… W2-T6: Data migration script â€” existing plain-text PII encrypted in dev DB
7. âœ… W2-T7: Integration tests â€” `CustomerEncryptionTests` 6+ cases PASS
8. âœ… W2-T8: `appsettings.Production.json` updated â€” `DataProtection:KeyDirectory`, `DataProtection:ApplicationName`
...
**PREVIOUS OBJECTIVE (archived)**
**Security Compliance â€” Wave 1: Notification Integration (Brevo Email + ESMS SMS)**

**Status:** âœ… COMPLETED â€” Branch `feature/wave1-notifications`, PR #39 merged to main

**Completed Actions:**
1. âœ… W1-T1: HttpClient used directly (no SDK â€” Brevo REST v3 + ESMS v4)
2. âœ… W1-T2: BrevoEmailService â€” IEmailService implementation, HTML support, error handling
3. âœ… W1-T3: EsmsNotificationService â€” ISmsService implementation, Unicode, 1 retry
4. âœ… W1-T4: CompositeNotificationService â€” INotificationService delegates to IEmailService + ISmsService
5. âœ… W1-T5: appsettings.Production.json + appsettings.Development.json with __REPLACE__ placeholders
6. âœ… W1-T6: 11/11 unit tests PASS (BrevoEmailServiceTests 5 + EsmsServiceTests 6)
...
**PREVIOUS OBJECTIVE (archived)**
**Security Compliance â€” Wave 0: JWT Authentication Foundation**

**Status:** âœ… COMPLETED (2026-06-23) â€” PR #38 merged to main
...
**Completed Actions:**
1. âœ… W0-T1: JwtBearer 8.0.8 + BCrypt.Net-Next 4.0.3 added to Central Package Management
2. âœ… W0-T2: IJwtTokenService + JwtTokenService created in 3_CoreHub/Services/
3. âœ… W0-T3: Login.cshtml.cs migrated to BCrypt.Verify + JWT cookie issue
4. âœ… W0-T4: AddJwtBearer added to Gateway/Program.cs (Cookie default + JwtBearer secondary)
5. âœ… W0-T5: ShopERP seed data: 5 DemoUsers with BCrypt hash work factor 12
6. âœ… W0-T6: 9 unit tests â€” JwtTokenServiceTests (6) + LoginPasswordTests (3) = 9/9 PASS
7. âœ… W0-T7: DevLoginController returns JWT token in response for E2E Bearer tests
8. âœ… W0-T8: CI fixes: ITenantProvider mock in ComponentTestBase (26/26 ShopERP tests) + flaky TamperedSignature test
...
**Status:** âœ… COMPLETED (2026-06-15)
...
```
KhachLink (5002) â†’ Gateway (5001) â†’ ShopERP (5003) â†’ SQLite Database
     â†“                  â†“                  â†“
...
**Completed Actions:**
1. âœ… Rolled back QrMenu.razor to use Gateway API (HttpClient) instead of IVanAnDbContext
2. âœ… Removed seed data from KhachLink Program.cs
3. âœ… Removed seed data from CoreHub Program.cs (Class Library)
4. âœ… Created ProductsController in ShopERP with IVanAnDbContext injection
5. âœ… Added seed data (5 products) to ShopERP Program.cs with TenantId: 00000000-0000-0000-0000-000000000001
6. âœ… Created Gateway ProductsController to forward requests to ShopERP via HttpClient
7. âœ… Fixed ShopERP DI issues (IAuditTrailService, IAuditLogRepository, ITenantProvider)
8. âœ… All services running: ShopERP (5003), Gateway (5001), KhachLink (5002)
9. âœ… API verification: curl http://localhost:5001/api/products?tenantId=... returns 200 OK with 5 products
10. âœ… Architecture tests: 7/7 PASS
11. âœ… Playwright E2E tests: 15 passed, 2 skipped
...
### BUCKET A GUEST CHECKOUT FORM + POSTGRESQL MIGRATION FIX â€” COMPLETE âœ…
**Commits:** `310f3da` + `8867dbc` on `main`

- **Guest checkout form:** `Order.SetCustomerInfo` + `CreateOrderCommand` + `OrderService` fix (CustomerId null) + `Checkout.razor` rewrite + `VanAInput @onchangeâ†’@oninput`
- **PostgreSQL migration:** `DesignTimeDbContextFactory` auto-detect provider + `PushSubscriptionConfiguration` `newid()â†’gen_random_uuid()` + regenerate `InitialCreate` with PG-native types + 36 tables
- **Tests:** Core.Tests 979/979 PASS, GuestCheckout 3/3 PASS, E2E qr-payment-ui 6/6 PASS

### E2E FIX â€” 4 PRE-EXISTING FAILURES RESOLVED âœ…
**Commit:** `24718b8` on `main`

- Tests 1-3: selector bug (`#qrPaymentModal .modal-content` instead of `#qrPaymentModal`)
- Test 4: IdentityModel v7.1.2 â€” added `IssuerSigningKeyResolver` to Gateway JWT
- Domain defect: `Order.SetCustomerInfo()` missing â€” user-approved fix

### STREAM G: SAAS PRODUCTION HARDENING â€” COMPLETE âœ… (W0-W7, W8 pending)
...
### STREAM F: VAS ENTERPRISE REPORTS â€” COMPLETE âœ… (W0-W9, 10 waves)
**Master plan:** `docs/AI/tasks/vas_enterprise_reports_master_plan.md`

- W0: Orderâ†’Accounting writer fix (9/18 issues)
- W1: Data audit + seed (31 journal entries, 5 account code fixes)
- W2: Domain records (3 enums, BCTC records, D9 HKDâ†”DN conversion)
...
### STREAM D: HKD BOOK ACCOUNTING FIX â€” COMPLETE âœ… (W0-W8, 12 waves)
...
### STREAM C: SHOPERP UI FIX â€” COMPLETE âœ… (W0-W6, 6 waves)
- 23 .razor files fixed, 14 dead pages â†’ 0, 18 unstyled â†’ 0, 3 broken layouts â†’ 0
- UI Platform compliance, CSS isolation, AdminLayout, governance cleanup

### STREAM B: E2E TEST CLEANUP â€” COMPLETE âœ… (W0-W8, 8 waves)
...
### ORDER LIFECYCLE STREAM â€” COMPLETE âœ… (W-1â†’W5 + edge cases)
- Sync mechanism (Outbox+NATS), SignalR, Kitchenâ†’Ready, Admin UI, Payment UI, polling, tests
- 8 edge case tests (idempotency, race condition, disconnected, partial completion, invalid payload)

### PLAYWRIGHT E2E GOLDEN TEST FIXES (W6) â€” COMPLETE âœ…
**Commit:** `fd7b038` â€” 21/22 PASS (1 deferred â†’ resolved by Bucket A)
...
1. **Blazor circuit crash** on `/`, `/sitemap`, `/admin/users` â€” `Authorization requires cascading parameter` â€” cascade timing issue
2. **DevLoginController role mismatch** â€” `/admin/users` requires "Owner" but `/dev/login/systemadmin` issues "SystemAdmin" â†’ Platform SystemAdmin plan addresses this (policy updates)
...
### TIERED AUTH PHASE 1-3 + PRODUCTION DEPLOY (2026-07-12 â†’ 2026-07-13)

**Tiered Auth Master Plan + 7 Task Cards (2026-07-12):**
- Created `tiered_auth_loyalty_master_plan.md` (7 phases, dependency graph, cost analysis â€” 96% saving)
...
- Strategy: Social Login (free) â†’ Zalo ZNS OTP (300Ä‘) â†’ eSMS fallback (1.000-1.200Ä‘)

**Phase 1 â€” Google OAuth (2026-07-13):**
...
- Dev secret rotation: scrubbed plain-text secret from `appsettings.Development.json` â†’ `dotnet user-secrets`
- Test fix: `AllShopErpControllers_MustHaveAuthCoverage` â€” added `HasClassLevelAllowAnonymous` skip
- Commit `b4c6aeb`

**Phase 2 â€” Verification Gate (2026-07-13):**
- `LoyaltyRewardsService.SubtractPointsAsync` â€” throws `IdentityLevelNotSufficientException` khi `IdentityLevel < Verified`
- Gate chá»‰ cho redeem, KHÃ”NG cho earn. Bug fix: `catch (IdentityLevelNotSufficientException) { rollback; throw; }` trÆ°á»›c generic catch
...
**Phase 3 â€” KhachLink UI (2026-07-13):**
- `SocialAuthHttpService.cs` (HTTP client cho upgrade + redeem)
- `Login.razor` â€” Google login button + OAuth callback handler
- `IdentityUpgradeModal.razor` â€” 3-step OTP upgrade flow (Intro â†’ OtpSent â†’ Success)
- `Profile.razor` â€” IdentityLevel badge + upgrade prompt
- `LoyaltyCard.razor` â€” redeem section + 403 â†’ show upgrade modal
...
### KHACHLINK FULL FLOW WAVES 0-4 (2026-07-11 â†’ 2026-07-12)
...
- Wave 0: Module Toggle Infrastructure â€” 6 toggles + Shop Settings UI + API + KhachLink HTTP service
...
- Pre-existing bug fix: `GetOrderByIdForPublicTrackingAsync` â€” `IgnoreQueryFilters` for anonymous endpoint
...
**Wave 4 â€” Configurable Polling Interval (2026-07-12):**
...
### ACCOUNTING POSTGRESQL ONLINE â€” 3 WAVES (2026-07-09 â†’ 2026-07-10)
...
**Wave 1 â€” Interface Split (2026-07-09):**
...
- ShopERP 500 crash: SQLite volume stale â†’ `DesignTimeDbContextFactory` + `MigrateAsync`
- KhachLink 500 crash: missing `Gateway__BaseUrl` â†’ added env var
- Dual Deployment Modes (SaaS + Edge) recorded in Section 5a
- Commits `9b2d209`, `b9ed4a2`

### ENTRY POINT CHECK + FIXES (2026-07-10 â†’ 2026-07-11)
...
- **Planning:** 2 role systems investigated (`UserRole` tenant-scoped vs `PlatformRole` cross-tenant). User chose pattern 2 lá»›p. Commit `792cc3f`.
