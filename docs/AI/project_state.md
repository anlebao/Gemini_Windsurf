...
| 2 | D | Fix wrong auth pattern (fill login form → use global storageState) | 6 | PENDING |
...
- **Branch:** `feature/e2e-cleanup-wave1-remove-reporter-pass` (off `main` @ `797ce36`)
- **Last commit:** `0c7965e` [E2E-CLEANUP WAVE 1] Pattern F: Remove decorative reporter.pass() calls
- **Build:** `dotnet build VanAn.sln` → 0 errors ✅ (verified post-Wave-1; .ts-only changes, C# unaffected)
- **Guard-check:** PASSED ✅
...
- **Uncommitted changes:** None on Wave 1 branch (only IDE .vs/ artifacts + stray local files: ci_local_output.txt, test_output*.txt, scripts/create-systemadmin.ps1)
...
- **In-progress:** Stream B Wave 1 complete; Wave 2 (fix auth pattern) ready to start next session.
...
3. **Wave 2: Fix auth pattern (Pattern D)** — NEXT. 6 files (`expense-entry-flow`, `export-excel-flow`, `einvoice-dashboard`, `invoice-management`, `provider-management`, `rbac-enforcement`) fill `#username`/`#email`/`#Username` login forms that don't exist (ShopERP uses OIDC + `/dev/login`). Remove form-fill `beforeEach`, rely on global `storageState`. Special: `rbac-enforcement` needs multi-role (`auth/staff.json`, `auth/owner.json`, etc.). Branch: `feature/e2e-cleanup-wave2-fix-auth-pattern`.
4. **Wave 3: Delete anti-schema tests (Pattern G1)** — `voice-command.spec.ts` (4 tests: hallucinated `result.Command.CommandText`, fabricated `tts-api.example.com`), `i18n.spec.ts` (1 test: `viResult.Executed`).
...
**Stream B — Wave 1 COMPLETE. Wave 2 next session.**
...
* **Last Updated:** 2026-07-03 — Stream B (E2E Test Cleanup) STARTED. Wave 0 pre-flight PASSED (auth/admin.json + playwright.config storageState + isTierEnabled verified, git clean, `npx playwright test --list` → 759 tests in 20 files). Wave 1 COMPLETE: removed 59 decorative `reporter.pass()` calls across 9 spec files via brace-balanced Node script (`6_Testing/scripts/wave1-remove-reporter-pass.js`); 4 comment refs preserved; imports/decls kept (reporter.log/setArchitectDecision still used); parse check unchanged (759 tests); `dotnet build` 0 errors; guard PASSED. Commit `0c7965e` on `feature/e2e-cleanup-wave1-remove-reporter-pass`. Pre-Stream-B hygiene commit `797ce36` (smoke test state). Next: Wave 2 (fix auth pattern in 6 files).
* **Current Branch:** `feature/e2e-cleanup-wave1-remove-reporter-pass`
* **Current Objective:** Stream B — E2E Test Cleanup. Wave 1 done, Wave 2 next.
