...
**Loyalty/CRM Audit Fix — P3 Cosmetic (COMPLETE 2026-07-28) → S7 Final (VPS RV + merge to main) IN PROGRESS**

P3-T1: Extracted `PromoPushComposer.razor` from `CustomerList.razor` inline modal. New component at `5_WebApps/ShopERP/Components/Pages/Admin/PromoPushComposer.razor` owns form fields (title/message/url) + reset-on-show. Parameters: `Show`, `RecipientCount`, `IsSubmitting`, `ErrorMessage`, `OnSubmit` (EventCallback<PromoPushPayload>), `OnClose`. `CustomerList.razor`: removed 56 lines of inline modal markup + 3 form-state fields + `ResetPromoForm` helper; `SubmitPromoAsync` now accepts `PromoPushComposer.PromoPushPayload`.
P3-T2: Extracted `PromoCampaignRecipientConfiguration.cs` to its own file at `3_CoreHub/Infrastructure/Configurations/PromoCampaignRecipientConfiguration.cs` (same namespace, same logic). `PromoCampaignConfiguration.cs` trimmed to single class.
P3-T3: Added `POST /api/customers/export` endpoint (`CustomerController.ExportCsv`). Accepts `SegmentRequest` body (same as `/segment`), returns CSV with columns `Name,Phone,Tier,Points,TotalSpent,LastOrder,Birthday,IdentityLevel,HasPush`. `Content-Type: text/csv`, `Content-Disposition: attachment; filename=customers.csv`. Minimal CSV escaping (quote-wrap on comma/quote/newline, doubles inner quotes).

Commit `018a42c2`. Build: 0 errors, 1117 warnings (pre-existing CA). Core.Tests 1038/1053 PASS (Release, 0 failed, 15 skipped — improved vs baseline 1023/1037). guard-check ALL PASSED (untracked files, encodings, windsurf-guard v6.0, architecture-guard v1.0, Roslyn, build, fast test gate). Pre-commit guard PASSED. No domain layer changes. Closes audit deviations D5, D7, D13.

**Next: S7 Final — VPS Runtime Verification (RV-AF-1 to RV-AF-16) + merge `fix/loyalty-crm-audit-fix` → `main`.**

Branch: `fix/loyalty-crm-audit-fix`. Commits: P0 `4aa0c6e2`, P1-T1 `2059f403`, P1-T2 `e58184da`, P1-T3 `756f1dac`, P2 `56926b44`, P3 `018a42c2`.
...
- **Last commit:** `018a42c2` feat(crm): AF-P3 cosmetic — extract PromoPushComposer + RecipientConfiguration + CSV export
- **.NET SDK:** 8.0.422
- **DB:** SQLite `vanan_shoperp.db` (business) + PostgreSQL `VanAnCoreHub` (accounting + Gateway + Community tables)
- **Build (2026-07-28):** 0 errors, 1117 warnings (pre-existing CA). Core.Tests 1038/1053 PASS (Release, 0 failed, 15 skipped). guard-check ALL PASSED (untracked files, encodings, windsurf-guard v6.0, architecture-guard v1.0, Roslyn, build, fast test gate). Pre-commit guard PASSED.
...
1. **Loyalty/CRM Audit Fix — Final (S7):** VPS Runtime Verification (RV-AF-1 through RV-AF-16) + merge `fix/loyalty-crm-audit-fix` to `main`. RV-AF-1 (build) + RV-AF-2 (guard-check) + RV-AF-15 (PromoPushComposer.razor exists) + RV-AF-16 (PromoCampaignRecipientConfiguration.cs exists) already PASS locally. RV-AF-10 to RV-AF-14 require VPS deployment (per-row promo send, bulk promo send, progress bar, detail expand, push column).
2. **Community Commerce Sprint 1 — Nearby Orders** — per `task_cc_sprint1_nearby_orders-2c5017.md`. Requires Domain Modification #2: OrderStatuses.Default[] + "delivering" status. **Needs user approval for Domain Modification.**
3. **Replace FingerprintJS stub** — Download real FingerprintJS v4 (MIT) before production deployment.
...
6. **(Cosmetic)** Fix `?` in Checkout.razor. Fix `isTabVisible` display bug in OrderTracking.razor.
7. **(Env)** Fix local DB role mismatch — ShopERP `vanan_admin` vs Gateway `vanan_dev`.
8. **(Guard-check script)** Investigate transient `$LASTEXITCODE` false-positive in fast-test-gate (`dotnet test ... | Out-Null` pattern). Direct `dotnet test` with identical args → exit 0; guard-check reports FAIL.
