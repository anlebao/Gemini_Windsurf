...
**[STREAM F: VAS ENTERPRISE FINANCIAL REPORTS (TT 99/2025 + TT 133/2016 + TT 58/2026) — W0+W1+W2+W3 MERGED TO MAIN, W4 ✅ COMPLETE (branch `feature/vas-wave4-services-bs-is-cf-tb`, pending merge)]**
...
- **Master plan:** `docs/AI/tasks/vas_enterprise_reports_master_plan.md` (W0+W1+W2+W3+W4 DONE, W5 next)
...
- **Next:** W5 (API endpoints — 4 controllers passthrough, response shape 2-column). Branch `feature/vas-wave5-api-controllers`. **JIT notes from master plan section 4.1:** Controllers passthrough — response shape đổi (2-column), không logic mới. Note 2-column response. TenantType filtering at controller level (Tenant.Type/AccountingStandard → standard param).
...
- **Branch:** `feature/vas-wave4-services-bs-is-cf-tb` (Stream F W4 COMPLETE, pending merge) · `main` (W0+W1+W2+W3 merged) · `feature/hkd-fix-wave8-ui-docx-export-regression` (Stream D Wave 8 Session 2 — parked)
- **VAS Stream (Stream F) — W4 COMPLETE:**
  - **Master plan:** `docs/AI/tasks/vas_enterprise_reports_master_plan.md` (W0-WW4 DONE, W5 next)
  - **10 task cards:** `vas_wave{0-9}_task_card.md` (W4 card updated with IMPLEMENT details + bugs fixed)
  - **W4 files created (8):** 4 interfaces + 4 implementations in `3_CoreHub/Services/`
  - **W4 files modified (2):** `5_WebApps/ShopERP/Program.cs` (+4 DI), `3_CoreHub/Services/HKDBookService.cs` ([Obsolete])
  - **W4 tests created (4 files, 25 tests):** BalanceSheetServiceTests (6), IncomeStatementServiceTests (6), CashFlowStatementServiceTests (6), TrialBalanceServiceTests (7)
  - **Build:** 0 errors, guard PASSED, W4 tests 25/25 PASS, Core.Tests 875/875 PASS, Arch.Tests 31/31 PASS
