...
- **Phase 3.5 (Accounting Consolidation):** Split `ConfirmPaymentAsync` into `MarkPaidAsync` (status=Paid + Outbox event) + `GenerateAccountingEntriesAsync` (public) + `ConfirmPaymentAsync` wrapper (POS backward compat). Gateway webhook → `MarkPaidAsync` → NATS `vanan.cloud.order.payment.confirmed.{shopInstanceId}` → ShopERP `PaymentConfirmedSubscriber` → entries in SQLite. Gateway `EInvoiceSyncSubscriber` for sync-back. OrderSyncSubscriber wildcard fix. SQLite compat fix (select only DefaultIndustrySector). Commits `653825c1` + `5d6d589d` + `7248ec2d`. VR 5/5 PASS on VPS (checkout → webhook → NATS → SQLite accounting entries generated).
- **Phase 4 (ShopERP OrderSyncSubscriber Routing):** `OrderSyncSubscriber.ExecuteAsync` now validates `ShopInstance:Id` (env var `SHOP_INSTANCE_ID` fallback) BEFORE NATS connect — throws `InvalidOperationException` if missing/invalid (fail-fast prevents cross-VPS data leak). Subscribes ONLY to routed subjects `vanan.cloud.order.created.{shopInstanceId}` + `vanan.cloud.order.status.changed.{shopInstanceId}` (wildcard/bare subscriptions removed). Product stub creation uses `UnitPrice` from payload instead of `0m`. Test seams: `protected virtual CreateSubscriptionConnection` + `RecordSubscription`. Config: `appsettings.json` (null default) + `appsettings.Development.json` (local dev Guid) + `docker-compose.yml` (env var). 5 new routing tests PASS. Core regression 1026/0/16. guard-check PASS. RV1/RV2 deferred to Phase 5 E2E (Phase 4 is subscriber-side only).
- **Phase 5 (KhachLink Multi-tenant Cart + Checkout UI + QR with Prices):** `CartItem.TenantId` (init-only, defaults Empty) + `CartState.AddItem` sets from `ProductDto.TenantId` + `QRCodePayload` adds `UnitPrice`/`VatRate`/`ProductName`/`TenantId` + `QrCodeService` 2 new overloads (6-arg, 7-arg) + `Scan.razor` fast path (no API call for new QR) + legacy fallback + `Checkout.razor` multi-tenant request + `CheckoutResponse` handling (loop orders, partial cart clear, `created_orders` localStorage) + `OrderTracking.razor` "other orders from session" section + `OrderHistory.razor` TenantId on OrderDto + ShopERP `ProductsController` QR generation passes price/VAT/name/tenantId + `ValidateProductPrice` endpoint + `Price_Validation_Enabled` toggle (entity + DTO + service + migration + UI) + `ProductManagement.razor` QR reminder. 12 new tests (5 CartItem + 5 QR Payload + 2 QR Service). Core regression 1038/0/16. guard-check PASS. **RV 9/9 PASS on VPS** (Gateway health + multi-tenant checkout + CheckoutResponse shape + QR PNG + ValidateProductPrice match=true/false + Price_Validation_Enabled toggle + KhachLink tracking UI). Commits `c38b51e5` + `e27727b1` + `e03bbebf` + `8f54270f` + `b2dc22c0` + `8718cb84`.
- **Phase 5 VPS deploy fixes (during RV):** (a) Npgsql `AddPriceValidationToggle` migration tried to drop FK already dropped by Phase 3 → removed duplicate DropFK/AddFK; (b) SQLite migration had unnecessary FK drop/re-add → removed (SQLite keeps FK); (c) `SHOP_INSTANCE_ID` env var missing from `docker-compose.prod.yml` → hardcoded Guid `00000000-0000-0000-0000-000000000001` (matches Phase 1 seed).
- **Phase 3.6 (Deferred Cleanup — COMPLETE):** OnboardingController refactor (removed `IIndustrySeedStrategy` seeding step from `TenantOnboardingService` — Gateway PG no longer stores Products per Phase 3 Option C; product seeding deferred to ShopERP QuickSetup run by tenant owner after first login) + Products forwarding port fix (added explicit `ShopERP__BaseUrl=http://shoperp:80/` env var to Gateway in `docker-compose.prod.yml` + `docker-compose.edge.yml` — prevents port 5003 fallback from `appsettings.Development.json` leak) + Architecture test fix (`VA-CONSISTENCY-004` exclusion for `SHOP_INSTANCE_ID`). Core.Tests 1036/0/16. Architecture 38/38. guard-check PASS. RV 2/2 PASS on VPS (products forwarding 200 OK + gateway health). Phase 5 regression 9/9 PASS. Commit `a6413668`.
- **Next:** Phase 6 (Admin UI) — per master plan.
...
- **Branch:** `main`
- **Last commit:** `a6413668` Phase 3.6: Deferred Cleanup — Onboarding refactor + Products forwarding port fix
...
- **Build (2026-07-20):** 0 errors. VanAn.sln build PASS. Core.Tests 1036/0/16. Architecture 38/38. guard-check PASS. CD pipeline run #29719644462 PASS. VPS: vanan-gateway + vanan-shoperp + vanan-khachlink all healthy.
...
**NEXT:** Phase 6 (Admin UI) — per `gateway_router_multi_vps_master_plan.md`. Task card: `phase6_admin_ui_task_card.md` (exists, needs scope review).

**Recommended order:** Phase 6 (Admin UI) → Phase 7 (Verification Governance) → Phase 8 (Multi-VPS E2E).

**Phase 3.6 COMPLETE (2026-07-20):** Onboarding refactor (removed product seeding — deferred to QuickSetup) + Products forwarding port fix (explicit `ShopERP__BaseUrl` env var). RV 2/2 PASS on VPS. Commit `a6413668`.
