...
**PLATE-AS-METADATA REFACTOR (PHASE 1) — COMPLETE + COMMITTED + DEPLOYED + RV L1-L3 PASS (L4 manual browser test pending).** Commit `154faf19` on `main` (pushed + fast-forward merged to `oracle-prod`). CD Multi-VPS run `32364470036` SUCCESS — all 3 VPS deployed + smoke tests PASS. RV L1-L3 on VPS: migration applied ✅, PlateNumber nullable ✅, all 3 VPS healthy ✅, Guard API 401 (feature enabled, not 503) ✅, Guard__QrVerifyEnabled=true ✅, 37 existing sessions intact (no data loss) ✅, no errors in logs ✅. L4 (browser UI flow — issue QR without plate, verify PrintTicket/Wallet display) pending manual test by user.

**GUARD QR FIXES — COMPLETE + COMMITTED + DEPLOYED (CD Multi-VPS SUCCESS).** Commit `d9ebd538` on `main` + `oracle-prod`. CD Multi-VPS run `32383443810` SUCCESS — all 6 jobs PASS (Build & Push + Pre-Deploy Validation + Deploy 3 VPS + Post-Deploy Smoke Test). Fixes: (1) Wallet short code — ClaimResult now returns ShortCode from server + DoClaimAsync extracts sc from {sc,sid} payload → wallet no longer shows "Vé #" with no code. (2) PrintTicket tenant info — fetches real tenant Name+Address+Phone via ITenantManagementService (was hardcoded "VẠN AN"). (3) PrintTicket QR payload now includes tn (tenantId) field → KhachLink Wallet can extract tenantId for anonymous users. (4) Scan.razor UI photo-first redesign — plate photo capture is PRIMARY (full-width), plate text + phone in compact optional row, customer photo collapsed into <details>, OCR status/hint/ROI debug hidden. (5) Step 2 "Kế tiếp" button for quick consecutive vehicle processing.
