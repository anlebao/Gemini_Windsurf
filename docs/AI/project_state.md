...
3. **Loyalty/CRM Audit Fix — merge to main (S7):** Branch `fix/loyalty-crm-audit-fix` (commits P0-P3, `4aa0c6e2`→`018a42c2`) đã verify local + VPS nhưng chưa merge to main. Cần merge + CD deploy + VPS RV (RV-AF-10 đến RV-AF-14: per-row promo, bulk promo, progress bar, detail expand, push column).
...
* **2026-07-28 — VPS CRM/LOYALTY VERIFICATION + P0/P1 FIX + A2 ROOT CAUSE FIX.** Commits `8d75abc1` + `1b02efb4` + `e47dad26` on `main` (DEPLOYED + VERIFIED). Verified `docs/user-guide/CRM_Loyalty_Guide.html` trên VPS `161.118.212.110` với 3 roles. Found 4 issues: (A1) Owner AccessDenied 3 trang admin — fix `[Authorize(Policy="SystemAdmin")]`→`OwnerOnly` (3 .razor files); (A2) Outbox stuck loop — root cause: EF Core SQLite Guid parameter UPPERCASE vs lowercase DB rows → SQLite BINARY collation case-sensitive → `rowsAffected=0` → loop. 4 evidence: rowsAffected=0 lowercase/=1 UPPERCASE, file path correct, WAL files exist, ToDomain reflection works. Fix: `OutboxRepository` raw SQL + `COLLATE NOCASE` (3 methods). 4 new file-based evidence tests (21/21 PASS). VPS verify: lowercase row processed <1s, 0 errors. (B1) Guide endpoint `/api/customer-orders`→`/api/customerorders` (2 doc files). (A3) `/sitemap` redirect by design. CD 3/3 jobs success. Branch: `main`.
