...
**IN PROGRESS:** Phase 5 (KhachLink UI multi-VPS checkout) — `phase5_khachlink_public_checkout_task_card.md`. Phase 4 COMPLETE: ShopERP `OrderSyncSubscriber` now subscribes only to routed NATS subjects `vanan.cloud.order.created.{shopInstanceId}` + `vanan.cloud.order.status.changed.{shopInstanceId}` (fail-fast if `SHOP_INSTANCE_ID` missing). Wildcard subscriptions removed — prevents cross-VPS data leak in multi-VPS deployment. Product stub creation uses `UnitPrice` from payload. 5 routing tests + 1026 Core regression PASS, guard-check PASS.
