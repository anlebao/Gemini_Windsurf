...
**Community Commerce Sprint 1 — IN PROGRESS (CC-S1-T0c COMPLETE, CC-S1-T0/T1/T2 pending)**

CC-S1-T0c (Customer Login Simplify) đã xong + VPS verified. Còn lại trong Sprint 1:
- **CC-S1-T0:** Domain Modification — add `"delivering"` vào `OrderStatuses.Default[]` + transition rules. **Needs user approval for Domain Modification.**
- **CC-S1-T1:** Nearby orders API (Haversine) — depends CC-S1-T0.
- **CC-S1-T2:** Accept order API + concurrency — depends CC-S1-T1.

Branch: `main`. Last commit: `4e7d9507` (CC-S1-T0c).
