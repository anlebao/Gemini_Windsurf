...
**[KHACHLINK FULL BUSINESS FLOW COMPLETION — WAVE 2 COMPLETE, WAVE 3 NEXT]**
...
**Wave 2 (COMPLETE ✅ — merged `e2a5f4d`):** Customer Confirm + Loyalty Bypass + PWA Disable + Accounting Bypass — 7 tasks. Live runtime verified (RV1-RV11 PASS). Pre-existing AuditLog tenant mismatch bug found (NOT Wave 2 regression — in AuditLogRepository.AddAsync when webhook sets tenant context).

**Wave 3 (NEXT):** Voice Note STT-only + TTS Kitchen + QR Table Number — 10 tasks. Task card: `docs/AI/tasks/khachlink_flow_wave3_voice_qr_task_card.md`. Branch (tbd): `feature/khachlink-flow-wave3-voice-qr`.
...
- **Branch:** `main` (Wave 2 merged). Wave 3 branch (tbd) pending.
- **Last commit:** `e2a5f4d` [KL WAVE 2] Merge: Customer confirm + loyalty bypass + PWA disable + accounting bypass COMPLETE
...
- **Completed streams (all merged to main):**
  - **KhachLink Wave 2: Customer Confirm + Loyalty Bypass + PWA Disable + Accounting Bypass ✅** (merge `e2a5f4d`) — 7 tasks. Live runtime verified. Pre-existing AuditLog tenant mismatch bug found (NOT regression).
...
**Immediate (KhachLink Full Flow — Wave 3):**
1. **Create branch** `feature/khachlink-flow-wave3-voice-qr` (from `main`).
2. **Wave 3 ANALYZE** — read voice note UI components, TTS kitchen display, QR table number flow. Investigate STT-only mode (no TTS for customer), TTS for kitchen staff, QR code generation for table numbers.
3. **Wave 3 IMPLEMENT** — 10 tasks: voice note STT-only (when toggle ON), TTS kitchen display (when kitchen ON), QR table number (when toggle ON), toggle checks for all 3 features.
4. **Wave 3 Live Runtime Verification (MANDATORY)** — RV1-RV12: boot ShopERP+KhachLink+Docker, test voice note upload, TTS kitchen display, QR table number generation, all 3 toggles ON/OFF. All MUST pass before mark COMPLETE.

**Wave 4 queued (task card ready, Live Runtime Verification Protocol embedded):**
...
* **2026-07-11 — KHACHLINK WAVE 2 COMPLETE + MERGED TO MAIN.** Wave 2 (Customer Confirm + Loyalty Bypass + PWA Disable + Accounting Bypass) implemented across 2 commits on `feature/khachlink-flow-wave2-completion-loyalty`, then merged to `main` via `e2a5f4d` (no-ff merge). 7 files (0 new + 7 modified): `OrderTracking.razor` (customer confirm button "Xác nhận đã nhận hàng" when status=ready + VanAnButton Success + ConfirmReceived method calling POST shoperp/api/orderworkflow/{id}/confirm-received + loyalty bypass VanAnAlert "Cảm ơn quý khách" when toggle OFF + loyaltyEnabled field + Loyalty_Program_Enabled toggle load), `PWAInstallPrompt.razor` (customer_token localStorage check + _isLoggedIn flag + GetDisplayStyle hide when logged in), `OrderWorkflowController.cs` (POST /api/orderworkflow/{orderId}/confirm-received endpoint + [AllowAnonymous] + TransitionStatusAsync routing), `OrderWorkflowService.cs` (state machine ready→delivered + delivered→completed/cancelled added to both kitchen ON and bypass tables), `OrderService.cs` (IShopFeatureSettingsService? optional constructor param + Accounting_Sync_Enabled toggle check before GenerateAccountingEntriesAsync + log when bypassed), `Gateway/Program.cs` (IShopFeatureSettingsService DI registration — was missing, needed for OrderService accounting bypass). **Live Runtime Verification (RV1-RV11 PASS):** boot Docker + ShopERP 5003 + KhachLink 5002 + Gateway 5001, customer confirm ready→delivered 200 + persisted, loyalty toggle OFF/ON persist, PWA disable for logged-in users, accounting bypass OFF (webhook 200, no AccountingEntries INSERT), accounting ON hits pre-existing AuditLog tenant mismatch bug (NOT Wave 2 regression — in AuditLogRepository.AddAsync when webhook sets tenant context). **Pre-existing bug found (NOT fixed — deferred):** `AuditLogRepository.AddAsync` line 141 throws `AuditLog tenant ID does not match current tenant` when webhook flow creates accounting entries. Root cause: webhook sets tenant via `_tenantProvider.SetTenant()` but AuditLogRepository uses a different tenant context. Not blocking Wave 2 (accounting bypass works correctly when toggle OFF). **Branch:** `main` (merged from `feature/khachlink-flow-wave2-completion-loyalty`).
