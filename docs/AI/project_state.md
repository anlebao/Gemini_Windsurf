...
- **Phase 1 ✅** Foundation (commit `bb7f72c0`): Domain records + BusinessProfile entity + EF config + migration `AddBusinessProfile` + repository + service + IncomeStatement extension (4 additive fields: TotalCogsEnding/Opening, TotalOpExEnding/Opening). 15/15 unit PASS.
- **Phase 2 ✅** Calculation Services (commit `52c55832`): 4 services (ProfitSummary, BreakEven single+multi, UnitEconomics, TargetProfit) + 6 guard codes inline (PROFILE_MISSING, INSUFFICIENT_DATA, COST_PRICE_MISSING, CM_RATIO_ZERO_OR_NEG, CAPACITY_EXCEEDED, FIXED_COST_ZERO). 37/37 unit PASS + 5/5 integration PASS (cross-check với real IncomeStatement: Revenue 10M/COGS 7M/OpEx 2M/Net 1M → BreakEvenRevenue 16.67M BelowBreakEven, TargetProfit 5M → RequiredRevenue 33.33M).
- **Phase 3 ✅** API + HTTP Proxy (commit `52c55832`): `FinancialIntelligenceController` (7 endpoints, class-level [Authorize JwtBearer] — W12-G7 compliant, W8 bypass: inject services trực tiếp cho HKD tenant) + `FinancialIntelligenceHttpService` (ShopERP proxy extends `GatewayAdminApiClientBase`, graceful degradation null on 4xx/5xx). DTOs trong `3_CoreHub/Services/FinancialIntelligence/Dtos/` namespace. 10/10 endpoint PASS + 9/9 arch PASS.
- **Phase 4 ✅** UI (commit `941e2fbd`): 4 Blazor pages (`/admin/business-profile` form, `/financial` dashboard 5 widgets, `/financial/break-even` single+multi table+bar chart+Export Excel, `/financial/unit-economics` sortable table+filter+warning highlight+Export Excel) + Period picker (month+year dropdown) + warning banners (PROFILE_MISSING/INSUFFICIENT_DATA/CM neg/FixedCost zero) + NavMenu "Thông tin tài chính" + Sitemap card "Phân tích tài chính" + UI Platform 100% (VanACard/VanAMetricsCard/VanAButton/VanAAlert) + i18n tiếng Việt có dấu (NFR-12) + Playwright E2E spec (Gate 4 — 10 tests covering all 4 pages + sitemap nav).
- **Phase 5 ✅** Polish (commit `941e2fbd`): `FinancialExportService` (EPPlus 7.6.1, precedent `InventoryExcelReport` Wave 3) — BreakEven + UnitEconomics .xlsx export via JS interop (`vanAn.downloadFile`). Admin guide doc `docs/admin-guide-financial-intelligence.md` (Owner-facing, 8 sections). Build full sln: 0 errors · Guard-check v7.2: ALL PASSED · Pre-commit guard PASSED.

**Next Actions:**
1. Push branch `feature/financial-intelligence-mvp2` (await user approval)
2. `gh pr create` — PR title "Financial Intelligence MVP-2 — All 5 phases" body summary + test plan
3. Merge PR → `main`
4. CD Multi-VPS deploy (`cd-multivps.yml` — push to `main` triggers deploy to all 3 VPS)
5. RV L1-L5 (API checks → static assets → Playwright runtime → UI flow → manual browser)
...
- **Branch:** `feature/financial-intelligence-mvp2` (forked from `main` @ `bb7f72c0`, 4 commits — all 5 phases complete, not pushed). **Build full sln:** 0 errors · **Guard-check v7.2:** ALL PASSED · **Financial Intelligence tests:** 22 unit (Phase 2 services) + 15 unit (Phase 1 BusinessProfile) + 5 service-integration + 10 endpoint-integration + 9 arch = **61/61 PASS** · **E2E spec:** `financial-dashboard.spec.ts` (10 tests, Gate 4 compliant, not yet run on VPS)
- **.NET SDK:** 8.0.422
- **Financial Intelligence MVP-2:**
  - Phase 1 ✅ Foundation (commit `bb7f72c0`): `BusinessProfile` entity (Single-Identity, 7 fixed costs + capacity + pricing + version), `FinancialModelVersion` VO, 5 result records (ProfitSummary, BreakEvenAnalysis, MultiProductBreakEven, UnitEconomicsReport, TargetProfitAnalysis), 2 enums, EF config `BusinessProfileConfiguration`, migration `20260821063126_AddBusinessProfile`, `IBusinessProfileRepository` + impl, `IBusinessProfileService` + impl, `IncomeStatement` record extended 4 additive fields (TotalCogsEnding/Opening, TotalOpExEnding/Opening). DI registered in Gateway. VasReportPageTestBase fixture updated.
  - Phase 2 ✅ Calculation Services (commit `52c55832`): 4 services (`ProfitSummaryService`, `BreakEvenAnalysisService` single+multi, `UnitEconomicsService`, `TargetProfitService`) trong namespace `VanAn.CoreHub.Services.FinancialIntelligence`. 6 guard codes inline với warning messages. CostPrice fallback 70% UnitPrice (matches OrderService.CalculateCogsAmount precedent). Pure deterministic (Trust Level 1). DI registered Gateway + IIncomeStatementService + IAccountChartService prerequisites.
  - Phase 3 ✅ API + HTTP Proxy (commit `52c55832`): `FinancialIntelligenceController` (7 endpoints, class-level `[Authorize(JwtBearer)]`, TenantId from JWT `tenant_id` claim, period parse `YYYY-MM`). DTOs namespace `VanAn.CoreHub.Services.FinancialIntelligence.Dtos` (8 DTOs, camelCase JSON). `FinancialIntelligenceHttpService` (ShopERP proxy, extends `GatewayAdminApiClientBase`, 7 methods, graceful degradation null on 4xx/5xx). DI registered in ShopERP. W8 feature flag bypass (HKD tenant có quyền truy cập). W12-G7 auto-pass. Pattern #10 audit (no forward variants → no charset strip needed).
  - Phase 4 ✅ UI (commit `941e2fbd`): 4 Blazor pages (`BusinessProfile.razor` form, `Dashboard.razor` 5 widgets, `BreakEven.razor` single+multi+chart+export, `UnitEconomics.razor` sortable+filter+export) + Period picker + warning banners + NavMenu "Thông tin tài chính" + Sitemap card "Phân tích tài chính" + UI Platform 100% + i18n tiếng Việt + Playwright E2E spec (Gate 4).
  - Phase 5 ✅ Polish (commit `941e2fbd`): `FinancialExportService` (EPPlus 7.6.1, .xlsx export via `vanAn.downloadFile` JS interop) + admin guide doc `docs/admin-guide-financial-intelligence.md`. Full sln build 0 errors. Guard-check v7.2 + pre-commit guard PASSED.
- **Previous status (Plate-as-metadata Phase 1 + Guard QR fixes):** ✅ COMPLETE + DEPLOYED + RV FULL PASS — on `main` @ `154faf19`. See archived history.
...
**Financial Intelligence MVP-2 — Phase 1+2+3 COMPLETE (staged, uncommitted). Next Actions:**
1. **(Commit Phase 2+3)** `git commit -m "feat(financial-intelligence): MVP-2 Phase 2+3 — calculation services + API + HTTP proxy + 47 tests"` trên branch `feature/financial-intelligence-mvp2`. Files staged: 8 service files + DTOs + controller + HTTP proxy + 4 unit test files + 2 integration test files + Program.cs edits.
2. **(Phase 4 — UI)** 4 Blazor pages:
   - `5_WebApps/ShopERP/Components/Pages/Admin/BusinessProfile.razor` + `.cs` — form (7 fixed costs + capacity + pricing + notes) VanAStatusForm
   - `5_WebApps/ShopERP/Components/Pages/Financial/Dashboard.razor` + `.cs` — 5 widgets (ProfitSummary, BreakEven status, Top 5, Bottom 5, Target Profit calculator) VanAMetricsCard
   - `5_WebApps/ShopERP/Components/Pages/Financial/BreakEven.razor` + `.cs` — single + multi-product table + chart + export PDF
   - `5_WebApps/ShopERP/Components/Pages/Financial/UnitEconomics.razor` + `.cs` — sortable table + filter by Category + warning row highlight + export Excel
   - Period picker (month + year dropdown) + warning banner (CostPrice missing, FixedCost zero, CM ratio negative) + NavMenu "Thông tin tài chính" + Sitemap card "Phân tích tài chính" + UI Platform compliance check (no raw HTML/CSS bypass, Gate 5)
3. **(Gate 4 — Playwright E2E)** `6_Tests/e2e-tests/financial-dashboard.spec.ts` — setup flow (create profile → redirect dashboard), dashboard render, target profit calculator, period switch
4. **(Phase 5 — Polish)** Export PDF (break-even + unit economics — use existing `RevenueExcelReport` / iTextSharp pattern), Export Excel (unit economics — use `InventoryExcelReport` pattern), i18n review (all labels tiếng Việt), admin guide doc (markdown), final guard-check + build + test gate
5. **(Commit Phase 4+5 + PR)** push branch → `gh pr create` → merge → CD Multi-VPS deploy
6. **(RV L1-L5)** L1 API (7 endpoints — 401/200/404/upsert), L2 Static (Blazor boot hash), L3 Playwright (dashboard render, target profit calc, period switch, export PDF/Excel), L4 Manual browser (Owner flow: create profile → dashboard → break-even → unit economics → target profit), L5 VPS SSH (container healthy, migration applied, BusinessProfiles table exists)
...
* **2026-08-21 — FINANCIAL INTELLIGENCE MVP-2 — PHASE 1+2+3 COMPLETE (Phase 2+3 staged, uncommitted).** Branch `feature/financial-intelligence-mvp2` (forked from `main` @ `bb7f72c0`). Phase 1 (Foundation) committed `bb7f72c0`: `BusinessProfile` entity (Single-Identity compliant — `Id = BusinessProfileId.Value`, `Ignore(BusinessProfileId)` in EF config, unique index TenantId) + `FinancialModelVersion` VO (Major.Minor string conversion) + 5 result records (`ProfitSummary`, `BreakEvenAnalysis`, `MultiProductBreakEven`, `UnitEconomicsReport`, `TargetProfitAnalysis`) + 2 enums (`ProfitStatus`, `BreakEvenStatus`) + `PricingModel` enum + `BusinessProfileConfiguration` (EF config) + migration `20260821063126_AddBusinessProfile` + `IBusinessProfileRepository` + impl + `IBusinessProfileService` + impl (Get/GetOrCreateDefault/Update with Version increment) + `IncomeStatement` record extended 4 additive fields (`TotalCogsEnding/Opening`, `TotalOpExEnding/Opening` — default = 0m, backward compat) + `IncomeStatementService` 2 return statements updated (TT99 mã 02/11 + flat path cogsEnding/opexEnding). 15 BusinessProfile unit tests PASS. VasReportPageTestBase fixture updated `TotalCogsEnding: 7_000_000m, TotalOpExEnding: 2_000_000m`. Phase 2 (Calculation Services — staged): 4 services trong namespace `VanAn.CoreHub.Services.FinancialIntelligence` — `ProfitSummaryService` (wrap `IIncomeStatementService`, classify Profitable/AtBreakEven/Loss/InsufficientData) + `BreakEvenAnalysisService` (single: `BreakEvenRevenue = FixedCost/CMRatio`, multi: weighted CM by sales mix, fallback 70% UnitPrice) + `UnitEconomicsService` (per-product ranking by `ProfitContribution DESC`, `HasMissingCostPrice` flag) + `TargetProfitService` (`RequiredRevenue = (FixedCost+TargetProfit)/CMRatio`, `RequiredDaily ≤ DailyCapacity` feasibility). 6 guard codes inline (`PROFILE_MISSING`, `INSUFFICIENT_DATA`, `COST_PRICE_MISSING`, `CM_RATIO_ZERO_OR_NEG`, `CAPACITY_EXCEEDED`, `FIXED_COST_ZERO`). Pure deterministic (Trust Level 1, NFR-14). DI registered Gateway + `IIncomeStatementService` + `IAccountChartService` prerequisites. 22 service unit tests + 5 service integration tests (cross-check real IncomeStatement: Revenue 10M/COGS 7M/OpEx 2M/Net 1M → BreakEvenRevenue 16.67M BelowBreakEven, TargetProfit 5M → RequiredRevenue 33.33M) PASS. Phase 3 (API + HTTP Proxy — staged): `FinancialIntelligenceController` (7 endpoints — GET/PUT business-profile, GET profit-summary, GET break-even, GET break-even/multi-product, GET unit-economics, POST target-profit; class-level `[Authorize(JwtBearer)]` — W12-G7 compliant; TenantId from JWT `tenant_id` claim; period parse `YYYY-MM`; W8 feature flag bypass — inject services trực tiếp cho HKD tenant, không qua `IncomeStatementsController`). DTOs namespace `VanAn.CoreHub.Services.FinancialIntelligence.Dtos` (8 DTOs camelCase JSON — chuyển từ Gateway Controllers/Dtos → CoreHub để ShopERP reference). `FinancialIntelligenceHttpService` (ShopERP proxy extends `GatewayAdminApiClientBase` — mints SystemAdmin JWT, 7 methods, graceful degradation null on 4xx/5xx/exception). DI registered ShopERP. 10 endpoint integration tests (401 no-auth, 404 missing profile, 200 upsert+GET, profit-summary cross-check 10M/7M/2M/1M, break-even InsufficientData, multi-product empty, unit-economics 0, target-profit PROFILE_MISSING, 400 invalid period, 401 missing tenant claim) + 9 arch tests (W12-G7 auto-pass) PASS. **Build full sln 0 errors, 1 warning (in-target). Guard-check v7.2 ALL PASSED** (Untracked, Encoding, Cross-provider SQL, Windsurf Guard, Architecture Guard, Roslyn, Build, Fast test gate). **Total MVP-2 tests: 61/61 PASS** (15 BusinessProfile + 22 service unit + 5 service integration + 10 endpoint integration + 9 arch). Next: commit Phase 2+3 → Phase 4 (UI: 4 Blazor pages + Period picker + warning banner + NavMenu + Sitemap + Playwright E2E Gate 4) → Phase 5 (Polish: Export PDF/Excel + i18n + admin guide doc) → PR → CD → RV L1-L5.
