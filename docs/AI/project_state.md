...
**[TIERED AUTHENTICATION FOR LOYALTY PROGRAM — P0+P1+P2+P3 ONLINE RV COMPLETE ✅ — P4 NEXT]**
...
**Phase 1 (COMPLETE ✅ + Production Wiring + Online RV 2026-07-13):** Google OAuth — `GoogleAuthService` + `SocialAuthController` + YARP route + Google credentials configured. **Online RV 3/3 PASS** (RV1: 302 redirect to Google with correct client_id, RV6: Gateway forwarding 302, RV7: invalid code → 302 to KhachLink with error param). RV2-RV5 pending (requires live Google OAuth flow test by user with real Google account). Production domain: `khachvip.online` (Google Cloud OAuth client created 2026-07-13, redirect URI `https://api.khachvip.online/api/auth/google/callback` configured).

**Phase 2 (COMPLETE ✅ + Online RV 2026-07-13):** Verification gate trong `SubtractPointsAsync` (throws `IdentityLevelNotSufficientException` khi `IdentityLevel < Verified`) + `POST /api/loyalty/redeem` endpoint (403 with `requiresUpgrade` payload) + `POST /api/customer-identity/upgrade/send-otp` + `POST /api/customer-identity/upgrade/verify-otp`. **Online RV 6/6 PASS** (RV1: redeem blocked for Social → 403 with requiresUpgrade=true, RV2: redeem success for Verified → 200 + points deducted, RV3: upgrade send OTP → 200, RV4: upgrade verify OTP → 200 + DB IdentityLevel=2, RV5: wrong OTP → 400 + IdentityLevel unchanged, RV6: earn points N/A — no earn API, unit test only). TDD: 6 unit tests PASS.

**Phase 3 (COMPLETE ✅ + Online RV 2026-07-13):** KhachLink UI: Google login button + token callback handler on Login.razor, IdentityUpgradeModal wired up with OTP upgrade flow, Profile.razor IdentityLevel badge, LoyaltyCard.razor redeem button + 403 → upgrade modal, SocialAuthHttpService. **Online RV 4/4 PASS** (RV1: login page accessible + Google button in DOM, RV4: profile page accessible, RV6: loyalty card page accessible at /my-loyalty). KhachLinkStartupTests 4/4 PASS.
...
- **Branch:** `main`
- **Last commit:** `4bd66bc1` [CD] Fix controller routes: kebab-case for customer-identity + loyalty
...
- **Tiered Auth Progress (2026-07-13):** P0 Domain ✅ · P1 Google OAuth + Production wiring + Online RV ✅ · P2 Verification Gate + Online RV ✅ · P3 KhachLink UI + Online RV ✅ · P4 Facebook OAuth ⬜ · P5 Zalo ZNS OTP ⬜ · P6 E2E Tests ⬜. All code committed + pushed. **Online RV 14/14 PASS on khachvip.online** (P1: 3/3, P2: 6/6, P3: 4/4 + 1 N/A).
- **Production Deploy (2026-07-13):** 7 CD runs (#1-#7) to fix deployment issues. Root causes found + fixed: (1) Dockerfiles missing `Directory.Packages.props` COPY → NuGet CPM restore failed → Google.Apis.Auth.dll missing from Docker image, (2) GHA cache stale layer → `no-cache: true` + removed `--mount=type=cache`, (3) `docker-compose.prod.yml` missing `ValidateProductionConfig` sentinel env vars (Jwt, DataProtection, Brevo, Esms, Redis, Authentication__ClientSecret), (4) Controller routes using `[controller]` token → PascalCase `CustomerIdentity` ≠ kebab-case `customer-identity` → route mismatch → 302 redirect to /Login. Final deploy: local build + SCP to VPS (bypass CD cache). PostgreSQL schema reset (migration conflict). All containers healthy on `khachvip.online`.
...
**Immediate (Tiered Auth — P0+P1+P2+P3 ONLINE RV COMPLETE ✅ — P4 next):**
1. ~~**Phase 0:** Domain `IdentityLevel` enum + `Customer.IdentityLevel` property + EF migration~~ ✅
2. ~~**Phase 1:** Google OAuth — redirect → callback → verify ID token → issue customer token~~ ✅ (Online RV 3/3 PASS; RV2-RV5 pending user live test with real Google account)
3. ~~**Phase 2:** Verification gate trong `SubtractPointsAsync` + upgrade OTP API~~ ✅ (Online RV 6/6 PASS)
4. ~~**Phase 3:** KhachLink UI — Google login button + upgrade modal + profile badge~~ ✅ (Online RV 4/4 PASS)
5. ~~**Online RV for P1+P2+P3 trên `khachvip.online`**~~ ✅ (14/14 PASS — 2026-07-13)
6. **USER ACTION:** Google Cloud Console — delete old leaked OAuth client (leaked secret in commit `72c815e`)
7. **USER ACTION:** Add GitHub secrets `GOOGLE_CLIENT_ID` + `GOOGLE_CLIENT_SECRET` (for future CD deploys to auto-write .env)
8. **USER ACTION:** Live Google OAuth flow test (RV2-RV5) — login with real Google account on `https://khachvip.online/login` → verify callback → token → profile redirect
9. **Phase 4:** Facebook OAuth — reuse `IGoogleAuthService` pattern (create `IFacebookAuthService` + `FacebookAuthService` + controller endpoints)
10. **Phase 5:** Zalo ZNS OTP (300đ/OTP) + CompositeOtpService (Zalo priority, eSMS fallback)
11. **Phase 6:** E2E Playwright tests — 2 scenarios (social login flow + OTP login flow)
...
* **2026-07-13 — TIERED AUTH P1+P2+P3 ONLINE RV COMPLETE ✅ (14/14 PASS) + PRODUCTION DEPLOY FIXES.** Deployed Tiered Auth Phase 1-3 to production VPS `khachvip.online`. **7 CD runs** to fix deployment issues: (1) Dockerfiles missing `Directory.Packages.props` COPY → NuGet CPM restore failed → `Google.Apis.Auth.dll` missing from Docker image → all `/api/*` endpoints 302 to /Login (controllers failed to load). (2) GHA cache stale layer → `no-cache: true` + removed `--mount=type=cache` from Dockerfile. (3) `docker-compose.prod.yml` missing `ValidateProductionConfig` sentinel env vars (Jwt, DataProtection, Brevo, Esms, Redis, Authentication__ClientSecret) → container crash loop. (4) Controller routes using `[controller]` token → PascalCase `CustomerIdentity` ≠ kebab-case `customer-identity` → route mismatch → 302 redirect. **Final deploy:** local build + SCP to VPS (bypass CD cache). PostgreSQL schema reset (migration conflict `AccountingEntries already exists`). **Online RV 14/14 PASS:** P1 (RV1: 302→Google with correct client_id, RV6: Gateway forwarding, RV7: invalid code→302 KhachLink error), P2 (RV1: redeem blocked Social→403 requiresUpgrade, RV2: redeem success Verified→200, RV3: upgrade send OTP→200, RV4: upgrade verify OTP→200+DB IdentityLevel=2, RV5: wrong OTP→400+unchanged, RV6: earn N/A), P3 (RV1: login page+Google button, RV4: profile page, RV6: loyalty card /my-loyalty). **Commits:** `a9cf334b`, `c7dd67bf`, `40392310`, `10e83f8f`, `1a9bbed4`, `23b8ef24`, `11cf6af6`, `4bd66bc1`. **Branch:** `main`. **User actions remaining:** (1) Delete old leaked OAuth client in Google Cloud Console, (2) Add GitHub secrets GOOGLE_CLIENT_ID+SECRET, (3) Live Google OAuth flow test (RV2-RV5) with real Google account.
