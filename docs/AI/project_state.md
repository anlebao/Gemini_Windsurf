...
**FINANCIAL INTELLIGENCE MVP-2 — Phase 1+2+3 COMPLETE (uncommitted). Phase 4 (UI) + Phase 5 (Polish) PENDING.**
- **Branch:** `feature/financial-intelligence-mvp2` (forked from `main` @ `bb7f72c0`)
...
- **Phase 2 ✅** Calculation Services (staged, uncommitted): 4 services (ProfitSummary, BreakEven single+multi, UnitEconomics, TargetProfit) + 6 guard codes inline (PROFILE_MISSING, INSUFFICIENT_DATA, COST_PRICE_MISSING, CM_RATIO_ZERO_OR_NEG, CAPACITY_EXCEEDED, FIXED_COST_ZERO). 37/37 unit PASS + 5/5 integration PASS (cross-check với real IncomeStatement: Revenue 10M/COGS 7M/OpEx 2M/Net 1M → BreakEvenRevenue 16.67M BelowBreakEven, TargetProfit 5M → RequiredRevenue 33.33M).
- **Phase 3 ✅** API + HTTP Proxy (staged, uncommitted): `FinancialIntelligenceController` (7 endpoints, class-level [Authorize JwtBearer] — W12-G7 compliant, W8 bypass: inject services trực tiếp cho HKD tenant) + `FinancialIntelligenceHttpService` (ShopERP proxy extends `GatewayAdminApiClientBase`, graceful degradation null on 4xx/5xx). DTOs trong `3_CoreHub/Services/FinancialIntelligence/Dtos/` namespace (chuyển từ Gateway → CoreHub để ShopERP reference). 10/10 endpoint PASS + 9/9 arch PASS.
- **Build full sln:** 0 errors, 1 warning (in-target) · **Guard-check v7.2:** ALL PASSED.

**Next Actions:**
1. Commit Phase 2+3 staged changes (git commit trên branch `feature/financial-intelligence-mvp2`)
2. Phase 4 — UI: 4 Blazor pages (`/admin/business-profile` form, `/financial` dashboard 5 widgets, `/financial/break-even` table+chart+PDF, `/financial/unit-economics` sortable table+filter+Excel) + Period picker + warning banner + NavMenu "Thông tin tài chính" + Sitemap card "Phân tích tài chính" + UI Platform compliance check + Playwright E2E (Gate 4 — UI layout change BẮT BUỘC viết E2E spec)
3. Phase 5 — Polish: Export PDF (break-even + unit economics — use existing iTextSharp pattern), Export Excel (unit economics — use InventoryExcelReport pattern), i18n review (tiếng Việt), admin guide doc (markdown), final guard-check + build + test gate
4. Sau Phase 5: commit Phase 4+5 → push → PR → CD Multi-VPS deploy → RV (L1-L5)
...
- **Branch:** `feature/financial-intelligence-mvp2` (forked from `main` @ `bb7f72c0`) — Phase 1 committed, Phase 2+3 staged (uncommitted). **Build:** 0 errors · **Guard-check v7.2:** ALL PASSED · **Financial Intelligence tests:** 22 unit (Phase 2 services) + 15 unit (Phase 1 BusinessProfile) + 5 service-integration + 10 endpoint-integration + 9 arch = **61/61 PASS**
- **.NET SDK:** 8.0.422
