...
**REALTIME PLATFORM — CHAT + LIVE LOCATION REUSABLE ACROSS MODULES — 🚧 APPROVED, IMPLEMENTING (session 2026-09-17/18) — P3 + P4 DEPLOYED · P5 CODE COMPLETE.**
...
- **Branch:** `main` @ `d8072ec5` (Realtime Platform P3 + P4 DEPLOYED + CD SUCCESS. Accounting revenue-loss fix + audit fail-safe deployed. Realtime Platform P2 — generic Domain/services + PG migration. P1b GPS map + Free/Charity checkout. Community Commerce Batch 1 + 2 + Chat + GPS + referral QR + commission base/self-referral. KhachLink Profile Sprint 1+2. GTM W2 + currency fix. W1 Merchant Audit + nginx Directory SSR. R2.2 Reseller Accounting — PR #169. Crawl-to-Onboard 8 phases. Issue #103/#157/#161/#156 deployed).
...
- [x] **P5** — ✅ **CODE COMPLETE (chưa deploy)** 2026-09-18: Shop chat + map tĩnh + inbox (xem Section 3). Logistics/JobMarket → **P7** (chưa tồn tại — F1). Chi tiết: task card Section 18.9.
...
- **Assumptions:** 0
- **Verified Facts:** Branch=`main` @ `7e732f04` (+ P4 working tree). Realtime Platform P4 (2026-09-18): UI Platform extraction đã verify — publish KhachLink có `_content/VanAn.UI.Platform/js/realtime.js` + `lib/leaflet/leaflet.{js,css}`; `RealtimeController` fallback Order→`IChatService.GetOrCreateConversationAsync` khi conversation miss (đơn mới không 404); `AddRealtimePlatform()` đăng ký 3 host; ChatHttpService xoá (chỉ còn Program.cs reference cũ — đã bỏ). 56/56 Realtime tests + Core.Tests 1635 PASS. Realtime Platform P2/P3 (2026-09-17): 11 findings F1-F11; unique index `(TenantId, SubjectType, SubjectId)`; generic hubs + `IRealtimeTokenValidator` × 3 (query-string-first — guest SignalR OK). `KhachLinkNavFlags.cs:52-53` vẫn `TODO R3` (Logistics/JobMarket chưa có entity — P7). Recent RV-verified work: commission base + self-referral (`05443115`) · referral QR (`aceab325`) · chat+GPS+QR (`26dc9e62`) · KhachLink UX (`32d0bae8`, `a6e712e3`) · Community Commerce Batch 2. Security: `.devin/rules/dev-token-secret.md` — `DevToken__Secret` unset (endpoint 404).
...
> **Last Updated:** 2026-09-18 · **Branch:** `main` @ `d8072ec5` + P5 uncommitted (Realtime Platform P5 code complete — commit pending)
