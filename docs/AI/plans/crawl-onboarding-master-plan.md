...
| M5 | Rate limit impl: `Microsoft.AspNetCore.RateLimiting` policy code cụ thể | Phase 4 | Dev |
| O1 | KhachLink đã có image upload service (Cloudinary)? | Phase 6 | Dev (check `5_WebApps/KhachLink/Services/Http/`) |
| O2 | Gateway đã có API key auth cho non-user clients? | Phase 5 | Dev (check `HmacApiKeyLookupAdapter.cs`) |
| O3 | `VanAnDbContextTestFactory` cần update cho 2 DbSet mới? | Phase 8 | Dev |
