...
> **Status:** ✅ DONE & MERGED (`feature/saas-w3-ci-pipeline-restore` → main, 2026-07-05)
...
## Verification
- [x] `.github/workflows/e2e.yml` — no `if: false` (removed from e2e-tests job)
- [x] `.github/workflows/ci.yml` — no `if: false` (removed from integration-tests job)
- [x] Auth files generated by global-setup: `auth/admin.json`, `auth/staff.json`, `auth/storekeeper.json`, `auth/guard.json`
- [x] `test.skip()` count reduced from 8 multi-role skips to 0 (all replaced with real tests)
- [x] CI pipeline: E2E job re-enabled with KhachLink service + env vars
- [x] CI pipeline: Integration job re-enabled with full suite (was CircuitBreaker-only filter)
...
2. **`global-setup.ts`:** Refactored to generate 4 auth files (admin, staff, storekeeper, guard) in a loop. Each role calls its specific `/dev/login/{role}` endpoint.
3. **`rbac-enforcement.spec.ts`:** Replaced 7 `test.skip()` with real multi-role tests using `test.use({ storageState: 'auth/<role>.json' })`. Tests verify Staff/StoreKeeper/Guard access denial and redirect behavior.
4. **`export-excel-flow.spec.ts`:** Replaced `test.skip()` with real Staff role test using `auth/staff.json`.
