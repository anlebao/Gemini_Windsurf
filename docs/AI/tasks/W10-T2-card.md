# TASK CARD: PRODUCTION_HYGIENE - WAVE10 - Delete LoyaltyRewardsService Interface

## 1. GOAL & CONTEXT
- **Mục tiêu cốt lõi:** Xóa ShopERP/Services/ILoyaltyRewardsService.cs - duplicate interface không được sử dụng
...
- **Files được phép đọc/sửa:**
  ...
  - `5_WebApps/ShopERP/Services/ILoyaltyRewardsService.cs` (xóa)
  - `docs/AI/tasks/PRODUCTION_HYGIENE_master_plan.md` (cập nhật status)
- **Boundary Rules (Nghiêm cấm):**
  - KHÔNG sửa CoreHub interface (3_CoreHub/Services/ILoyaltyRewardsService.cs)
  - KHÔNG sửa implementation (3_CoreHub/Services/LoyaltyRewardsService.cs)
...
- [ ] **SC1:** ILoyaltyRewardsService.cs (ShopERP) đã xóa khỏi repository
- [ ] **SC2:** Không có code references đến ShopERP ILoyaltyRewardsService
...
- [ ] **SC7:** PRODUCTION_HYGIENE_master_plan.md updated với W10-T2 status = ✅ DONE
...
- **Evidence Count:** 3
- **Verified Facts:**
  - Fact 1: ShopERP ILoyaltyRewardsService là duplicate của CoreHub interface
...
|---|---|---|
| ILoyaltyRewardsService.cs (xóa) | Không có reverse impact | CoreHub interface is source of truth |
...
### Micro-phase breakdown cho WAVE10 - Delete LoyaltyRewardsService Interface

| Session | JIT Planning (chốt gì) | Pure Execution (viết gì) |
|---|---|---|
| **S1** | Verify no references to ShopERP ILoyaltyRewardsService | Execute grep search, analyze results, confirm safe to delete |
| **S2** | Delete ILoyaltyRewardsService.cs file | Execute file deletion, verify file removed |
