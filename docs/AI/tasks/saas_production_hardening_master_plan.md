...
| W2 | .NET SDK Upgrade + Package Security | IMPLEMENT | 1 | `saas_w2_task_card.md` | ✅ PARTIAL — Removed 9 legacy 2.3.0 auth packages + FrameworkReference for .NET 8 shared framework. **SDK 8.0.422 installed user-local but NOT resolved by system dotnet** (PATH/multilevel lookup issue — system `C:\Program Files\dotnet\` first in PATH, only has 8.0.100). global.json `rollForward: latestFeature` correctly configured but system dotnet can't see user-local SDK. Fix: reorder PATH or install 8.0.422 to system path. 1133/1133 tests PASS. |
...
| W8 | Final Regression + Production Tag | REVIEW | 3 | `saas_w8_task_card.md` | ✅ DONE — Full regression 1257 tests PASS (Core 941 + Arch 34 + Integration 177 + ShopERP 99 + Load 6; E2E 27 CI-only). Build 0 errors (SDK 8.0.100 active — 8.0.422 installed user-local but not resolved). guard-check ALL PASSED. 3/4 blockers resolved (B1 Option B, B2 env vars, B4 CI active; B3 SDK PATH issue unresolved). All 5 hardening items verified (H5 deferred — credentials). Tech debt M1/M4/M6/M7 done, M2/M3/M5 deferred. Tag `saas-production-v1.0` created. Smoke test deferred to user. |
