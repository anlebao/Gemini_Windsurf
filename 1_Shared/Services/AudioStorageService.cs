...
public partial class AudioStorageService : IAudioStorageService
{
    ...
    {
        _logger = logger;
        _audioFiles = new Dictionary<string, AudioFile>();
    ...
    public async Task<AudioFile> SaveAudioAsync(byte[] audioData, string fileName, string orderId)
    {
        try
        {
            ...
            var filePath = Path.Combine(_storagePath, $"{audioId}_{fileName}");
    ...
    public async Task<bool> DeleteAudioAsync(string audioId)
    {
        try
        {
            if (_audioFiles.TryGetValue(audioId, out var audioFile))
            {
                // Delete file from disk
                var filePath = Path.Combine(_storagePath, audioFile.FileName);
    ...
    public async Task<AudioFile?> GetAudioAsync(string audioId)
    {
        ...
        {
            await Task.CompletedTask;
            return _audioFiles.TryGetValue(audioId, out var audioFile) ? audioFile : null;
        }
        catch (Exception ex)
        {
    ...
    public async Task<bool> CleanupExpiredAudioFilesAsync()
    {
        try
        {
            var expiredFiles = await GetExpiredAudioFilesAsync();
            var deletedCount = 0;
