namespace VanAn.Shared.Domain
{
    ...
    public record S1aHKDTemplate : HKDBookTemplate
    {
        ...
        {
            var report = $"SỔ KẾ TOÁN S1a_HKD - {book.Period.Year}/{book.Period.Month:D2}\n";
    ...
    public record S2aHKDTemplate : HKDBookTemplate
    {
        ...
        public override async Task<string> GenerateReportAsync(GenericHKDBook book)
        {
            var report = $"SỔ KẾ TOÁN S2a_HKD - {book.Period.Year}/{book.Period.Month:D2}\n";
            report += $"Hộ kinh doanh: {book.TenantId.Value}\n";
    ...
    public record S2bHKDTemplate : HKDBookTemplate
    {
        ...
        public override async Task<string> GenerateReportAsync(GenericHKDBook book)
        {
            var report = $"SỔ DOANH THU S2b_HKD - {book.Period.Year}/{book.Period.Month:D2}\n";
            report += $"Hộ kinh doanh: {book.TenantId.Value}\n";
            
            if (book.NumericValues.TryGetValue("SalesRevenue", out var sales))
                report += $"Doanh thu bán hàng: {sales:N0} VNĐ\n";
            
            if (book.NumericValues.TryGetValue("ServiceRevenue", out var service))
                report += $"Doanh thu dịch vụ: {service:N0} VNĐ\n";
            
            if (book.NumericValues.TryGetValue("TotalRevenue", out var total))
    ...
    public record S2cHKDTemplate : HKDBookTemplate
    {
        ...
        public override async Task<string> GenerateReportAsync(GenericHKDBook book)
        {
            var report = $"SỔ CHI TIẾT DOANH THU, CHI PHÍ S2c_HKD - {book.Period.Year}/{book.Period.Month:D2}\n";
            report += $"Hộ kinh doanh: {book.TenantId.Value}\n";
            
            if (book.NumericValues.TryGetValue("Revenue", out var revenue))
                report += $"Doanh thu: {revenue:N0} VNĐ\n";
            
            if (book.NumericValues.TryGetValue("CostOfGoodsSold", out var cogs))
    ...
    public record S2dHKDTemplate : HKDBookTemplate
    {
        ...
        public override async Task<string> GenerateReportAsync(GenericHKDBook book)
        {
            var report = $"SỔ CHI TIẾT VẬT LIỆU, DỤNG CỤ, SẢN PHẨM, HÀNG HÓA S2d_HKD - {book.Period.Year}/{book.Period.Month:D2}\n";
            report += $"Hộ kinh doanh: {book.TenantId.Value}\n";
            ...
            if (book.NumericValues.TryGetValue("TotalInventory", out var total))
    ...
    public record S2eHKDTemplate : HKDBookTemplate
    {
        ...
        public override async Task<string> GenerateReportAsync(GenericHKDBook book)
        {
            var report = $"SỔ CHI TIẾT TIỀN S2e_HKD - {book.Period.Year}/{book.Period.Month:D2}\n";
            report += $"Hộ kinh doanh: {book.TenantId.Value}\n";
            
            if (book.NumericValues.TryGetValue("CashOnHand", out var cash))
                report += $"Tiền mặt: {cash:N0} VNĐ\n";
            
            if (book.NumericValues.TryGetValue("BankDeposits", out var bank))
                report += $"Tiền gửi ngân hàng: {bank:N0} VNĐ\n";
            
            if (book.NumericValues.TryGetValue("TotalCash", out var total))
    ...
    public record S3aHKDTemplate : HKDBookTemplate
    {
        ...
        public override async Task<string> GenerateReportAsync(GenericHKDBook book)
        {
            var report = $"SỔ THUẾ KHÁC S3a_HKD - {book.Period.Year}/{book.Period.Month:D2}\n";
            report += $"Hộ kinh doanh: {book.TenantId.Value}\n";
