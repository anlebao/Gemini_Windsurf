...
namespace VanAn.Shared.Domain.Aggregates.TenantAggregate
{
    ...
    public class Tenant : AggregateRoot
    {
        ...
        // Crawl-to-Onboard Pipeline (2026-08-25): Flag indicating this Pending tenant
        // is a potential duplicate of another tenant (same MST). SysAdmin must resolve
        // (pick one to Verify, other → Inactive) before this tenant can be Verified.
        ...
        /// <summary>
        /// Crawl-to-Onboard Pipeline (2026-08-25): Create a Pending tenant from crawled business listing.
        /// Status=Pending=5 (correction H1 — not 0, avoid EF default sentinel).
        /// Raises TenantPendingEvent (NOT TenantCreatedEvent — no welcome email, tenant not yet verified).
        ///
        /// Pending scope: profile read-only public + Claim button. No login, no orders, no accounting.
        /// SĐT section HIDDEN on profile (M3 resolved — CrawledPhone stored internal, NOT displayed).
        ///
        /// pendingSlug: auto-generated by service layer as `pending-{taxCode}-{random4}`.
        /// Set directly on Settings (bypass UpdateSlug() — correction C4: UpdateSlug guard unchanged,
        /// only allows Inactive guard; Pending bypasses via this factory parameter).
        /// Slug validation inlined here (same regex as UpdateSlug) to enforce domain invariants.
        /// </summary>
        public static Tenant CreateUnverified(TenantId id, string name, TenantSettings? settings, string pendingSlug)
        {
            ArgumentException.ThrowIfNullOrWhiteSpace(name);
            ArgumentException.ThrowIfNullOrWhiteSpace(pendingSlug);

            // Inline slug validation (same rules as UpdateSlug — DRY would be nice but
            // UpdateSlug has Status guard we want to bypass for Pending tenants)
            pendingSlug = pendingSlug.Trim().ToLowerInvariant();
            if (pendingSlug.Length > 100)
                throw new ArgumentException("Pending slug must be at most 100 characters.", nameof(pendingSlug));
            if (!System.Text.RegularExpressions.Regex.IsMatch(pendingSlug, @"^[a-z0-9]+(?:-[a-z0-9]+)*$"))
                throw new ArgumentException("Pending slug must be lowercase, alphanumeric, hyphen-separated.", nameof(pendingSlug));

            var baseSettings = settings ?? TenantSettings.Empty();
            var tenant = new Tenant
            {
                Id = id,
                Name = name,
                BusinessType = BusinessType.Company,  // crawled businesses are Companies
                Status = TenantStatus.Pending,  // H1: Pending=5
                Settings = baseSettings.WithSlug(pendingSlug)
            };
            tenant.SetTenantId(id);
            tenant.AddDomainEvent(new TenantPendingEvent(id.Value, name, baseSettings.TaxCode, null, DateTime.UtcNow));
            return tenant;
        }
        ...
        /// <summary>
        /// Crawl-to-Onboard Pipeline (2026-08-25): Verify a Pending tenant → Active.
        /// Called by VerifyAsync service after owner Claim + SysAdmin Approve.
        /// Raises TenantVerifiedEvent (will trigger NATS sync sang SQLite — Option A).
        ///
        /// Guards (correction H4):
        /// - Status must be Pending (cannot verify Active/Suspended/Inactive/Converted).
        /// - PotentialDuplicateOf must be null — SysAdmin must resolve duplicate first
        ///   (via ResolveDuplicateAsync: pick one to Verify, other → Inactive).
        ///
        /// Post-Verify (service layer responsibility, NOT domain):
        /// - Create owner user + Owner role + 4 permission groups
        /// - Set ContactPhone from owner-provided Claim form (consented — M3)
        /// - Update slug to clean slug (now UpdateSlug() works — tenant is Active)
        /// - Publish OutboxMessage TenantVerifiedEvent → NATS → TenantSyncSubscriber upsert SQLite
        /// </summary>
        public void Verify()
        {
            if (Status != TenantStatus.Pending)
                throw new InvalidOperationException($"Cannot verify tenant in status {Status}. Only Pending tenants can be verified.");
            if (PotentialDuplicateOf is not null)
                throw new InvalidOperationException("Cannot verify tenant flagged as potential duplicate. SysAdmin must resolve duplicate first (pick one to Verify, other → Inactive).");

            Status = TenantStatus.Active;
            UpdateAudit();
            AddDomainEvent(new TenantVerifiedEvent(Id.Value, Guid.Empty, DateTime.UtcNow));
            // Note: ApprovedByUserId set by service layer (passes actual SysAdmin user id)
            // — domain event uses Guid.Empty as placeholder, service enriches before outbox publish.
        }

        /// <summary>
        /// Crawl-to-Onboard Pipeline (2026-08-25): Mark this Pending tenant as potential duplicate
        /// of another tenant (same MST found in existing Active/Pending tenant).
        /// Correction H5: service layer ensures first canonical tenant is kept, rest are marked
        /// duplicate of canonical (not chain of duplicates).
        /// No domain event — internal flag for SysAdmin review.
        /// </summary>
        public void MarkPotentialDuplicateOf(Guid otherTenantId)
        {
            if (otherTenantId == Guid.Empty)
                throw new ArgumentException("Other tenant id cannot be empty.", nameof(otherTenantId));
            if (otherTenantId == Id.Value)
        ...
        public bool IsPending() => Status == TenantStatus.Pending;  // Crawl-to-Onboard (2026-08-25)
