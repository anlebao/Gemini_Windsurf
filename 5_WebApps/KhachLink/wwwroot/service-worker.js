// ============================================================================
// VanAn KhachLink PWA Service Worker â€” Phase 3 + SRI Hotfix (v12-sri-fix)
// ============================================================================
// Cache strategy:
//   - _framework/*.wasm/.dll/.js â†’ network-first + cache fallback (WASM_CACHE)
//     Â· Was cache-first â€” caused SRI mismatch after deploys (stale cached wasm
...
//       as cached boot.json â†’ SRI passes).
//   - blazor.boot.json â†’ network-first + cache fallback (detect updates)
//   - Static assets (CSS/JS/icons) â†’ cache-first (STATIC_CACHE)
//   - API GETs (whitelisted) â†’ network-first + cache fallback (DYNAMIC_CACHE)
//     Â· catalog/campaigns â†’ stale-while-revalidate (show cached, refresh bg)
//     Â· 24h cache expiration via x-sw-cached-at header
//   - Navigation â†’ network-first â†’ cache â†’ offline shell
//
// Phase 3 changes:
...
//   - Cache version bumped v10-batched â†’ v11-phase3
//
// SRI Hotfix (v12-sri-fix, 2026-07-22):
//   - WASM/DLL assets changed from cache-first â†’ network-first + cache fallback
//     (cache-first caused SRI integrity mismatch after deploys: stale cached wasm
//     vs fresh blazor.boot.json integrity hashes â†’ browser blocked wasm load)
//   - Added activate event to delete stale caches from old SW versions
//     (caches.match() checks ALL caches â€” old entries caused SRI mismatches)
//   - Cache version bumped v11-phase3 â†’ v12-sri-fix
// ============================================================================
...
// Core static assets to cache (must all return 200 â€” addAll fails on any 404)
...
// Dynamic API content that can be cached (Option C endpoints â€” verified against
...
self.addEventListener('install', event => {
  event.waitUntil(
    Promise.all([
      ...
      // Batched to avoid overwhelming nginx rate limiter (burst=20 â†’ 503 on 80 concurrent).
      ...
      console.log('SW install complete â€” activating');
...
// (e.g., v10-batched, v11-phase3) â€” causing SRI mismatches after deploys.
...
// expired (or if timestamp missing â€” defensive, treat unset as expired).
...
self.addEventListener('fetch', event => {
  ...
  // Only intercept GET requests â€” POST/PUT/DELETE go straight to network
  ...
  // integrity hashes. Was cache-first â€” caused SRI mismatch after deploys because
  // stale cached wasm (old build) was returned with fresh boot.json (new hashes).
  // Offline fallback: cached wasm from same deploy as cached boot.json â†’ SRI passes.
  ...
  // endpoints (/api/customers/me, /api/loyalty/my) are NOT cached â€” avoids
  ...
  if (isCacheable) {
    ...
    if (isSwr) {
      ...
      // If cached is still fresh (< 24h), skip background fetch entirely â€” no
      ...
      event.respondWith(
        caches.match(request).then(cachedResponse => {
          ...
          const fetchPromise = fetch(request)
            ...
            .catch(() => {
              // Network failed â€” if no cached response, return offline JSON
              if (!cachedResponse) {
                return new Response(
  ...
  // --- 5. Navigation (HTML pages): network-first â†’ cache â†’ offline shell ---
...
// (shown only when ALL fallbacks fail â€” WASM cached = app loads normally offline)
// ============================================================================
...
<head>
  ...
  <title>Váº¡n An â€” Ngoáº¡i tuyáº¿n</title>
  <style>
...
<body>
  <div class="offline-card">
    <div class="logo">
      <img src="/icons/icon-192x192.png" alt="Váº¡n An" onerror="this.style.display='none'">
    </div>
    <div class="icon-offline" aria-hidden="true">ðŸ“¡</div>
    <h1>Báº¡n Ä‘ang ngoáº¡i tuyáº¿n</h1>
    <p>App Váº¡n An cáº§n káº¿t ná»‘i internet Ä‘á»ƒ Ä‘áº·t hÃ ng vÃ  xem cá»­a hÃ ng.<br>Vui lÃ²ng kiá»ƒm tra máº¡ng vÃ  thá»­ láº¡i.</p>
    <button class="retry-btn" onclick="window.location.reload()">Thá»­ láº¡i</button>
    <div class="footer">Váº¡n An Group â€” Há»‡ thá»‘ng Ä‘áº·t hÃ ng thÃ´ng minh</div>
  </div>
</body>
</html>`;
...
self.addEventListener('activate', event => {
  ...
  event.waitUntil(
    caches.keys().then(cacheNames => {
      ...
      console.log('SW activated â€” Phase 3 offline API fallback hardening ready');
...
self.addEventListener('push', event => {
  let notificationData = {
    title: 'Váº¡n An Group',
    body: 'Báº¡n cÃ³ thÃ´ng bÃ¡o má»›i tá»« Váº¡n An Group',
