...
public class ConflictResolutionService : IConflictResolutionService
{
    ...
    private readonly ISyncConflictResolver _baseResolver;
    
    public ConflictResolutionService(
        ILogger<ConflictResolutionService> logger,
        ISyncConflictResolver baseResolver)
    {
        _logger = logger;
    ...
    public async Task<ResolutionResult> ResolveOrderConflictAsync(OfflineOrderDto offlineOrder, Order? serverOrder)
    {
        ...
        try
        {
            ...
            var baseResolution = await _baseResolver.ResolveOrderConflictAsync(offlineOrder, serverOrder);
    ...
    public async Task<ResolutionResult> ResolveCartConflictAsync(List<OfflineOrderItemDto> offlineItems, List<CartItem> serverItems)
    {
        ...
        try
        {
            ...
            // Use base resolver
            var baseResolution = await _baseResolver.ResolveCartConflictAsync(offlineItems, serverItems);
    ...
    public async Task<ConflictReport> GenerateConflictReportAsync(OfflineOrderDto offlineOrder, Order? serverOrder)
    {
        ...
        try
        {
            ...
            // Items conflict
            var offlineProductIds = offlineOrder.Items.Select(i => i.ProductId).ToHashSet();
            var serverProductIds = serverOrder.Items.Select(i => i.ProductId.ToString()).ToHashSet();
    ...
    private async Task<ResolutionResult> ProcessUseOfflineAsync(OfflineOrderDto offlineOrder, Order? serverOrder)
    {
        var result = new ResolutionResult
        {
            Success = true,
    ...
    private async Task<ResolutionResult> ProcessUseServerAsync(OfflineOrderDto offlineOrder, Order? serverOrder)
    {
        var result = new ResolutionResult
        {
            Success = true,
    ...
    private async Task CheckCartWarningsAsync(List<OfflineOrderItemDto> items, ResolutionResult result)
    {
        if (items.Count > 20)
        {
        ...
        var highQuantityItems = items.Where(i => i.Quantity > 10).ToList();
        if (highQuantityItems.Any())
        {
